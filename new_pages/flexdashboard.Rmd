...
* Nó yêu cầu tối thiểu mã hóa R không chuẩn - với rất ít thực hành, bạn có thể nhanh chóng tạo một dashboard  
...
### Tải các packages {.unnumbered}  

Trong sổ tay này, chúng tôi nhấn mạnh đến `p_load()` từ **pacman**, cài đặt package nếu cần *và* tải nó để sử dụng. Bạn cũng có thể tải các package đã cài đặt với `library()` từ **base** R. Xem chương trên [R cơ bản] để biết thêm thông tin về các R packages.  
...
Script là một script R Markdown và do đó có các thành phần và tổ chức tương tự như được mô tả trong chương [Báo cáo với R Markdown]. Chúng tôi xem lại một cách ngắn gọn các định dạng này và nêu rõ sự khác biệt so với các định dạng đầu ra R Markdown khác.  
