...
7. Biểu thức thông thường ("regex")
...
Package [stringr vignette](
https://cran.r-project.org/web/packages/stringr/vignettes/stringr.html) này đã cung cấp nhiều cảm hứng cho chương này. 
...
Argument `sep = ` chèn một giá dạng trị ký tự vào giữa mỗi argument (ví dụ: chèn dấu phẩy, dấu cách hoặc dòng mới `"\n"`)  
...
YBạn cũng có thể điều chỉnh số phần tách ra với argument `n = `. Ví dụ: điều này hạn chế số phần  tách ra chỉ có 2 phần. Bất kỳ dấu phẩy nào khác vẫn sẽ nằm trong giá trị thứ hai. 
...
<span style="color: orange;">**Chú ý:_** Nếu bạn không cung cấp đủ giá trị `cho`các cột mới, dữ liệu của bạn có thể bị cắt bớt .</span>  
