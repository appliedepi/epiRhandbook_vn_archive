...
Chương này bao gồm hai phương pháp tính toán và biểu diễn những đường trung bình động:

1)  Tính toán với package **slider**\
2)  Tính toán *trong* lệnh `ggplot()` với package **tidyquant**
...
Package **slider** cung cấp một số hàm trong "cửa sổ trượt" giúp tính toán trung bình động, tổng tích lũy, hồi quy động, v.v. Nó coi data frame như một vectơ của các hàng, cho phép lặp lại hàng qua một data frame.

Dưới đây là một số hàm phổ biến:

-   `slide_dbl()` - lặp qua một cột *số* (từ "\_dbl") để thực hiện thao tác sử dụng cửa sổ trượt

    -   `slide_sum()` - hàm tắt tính tổng động cho `slide_dbl()`\
    -   `slide_mean()` - hàm tắt tính trung bình động cho `slide_dbl()`

-   `slide_index_dbl()` - áp dụng cửa sổ động trên một cột số bằng cách sử dụng một cột riêng biệt để *lập chỉ mục* cửa sổ tiến trình (hữu ích nếu tính toán dữ liệu biến động theo ngày mà một số ngày bị thiếu)
...
-   `.i =` cho các phiên bản "chỉ mục (index)" của hàm **slider** - cung cấp một cột để "lập chỉ mục" cho dữ liệu động (xem phần [dưới đây](#roll_index))\
...
Ví dụ: Để có cửa sổ 7 ngày liên tục bao gồm giá trị hiện tại và sáu giá trị trước đó, hãy sử dụng `.before = 6`. Để có cửa sổ "căn giữa" hãy cung cấp cùng một giá trị số cho cả `.before =` và `.after =`.
...
### Biến động theo ngày {#roll_index .unnumbered}

Trường hợp sử dụng có khả năng xảy ra nhất của phép tính động trong dịch tễ học ứng dụng là kiểm tra một số liệu *theo thời gian*. Ví dụ: đo lường động tỷ lệ trường hợp, dựa trên số lượng trường hợp hàng ngày.

Nếu bạn có dữ liệu chuỗi thời gian đã được làm sạch với đủ giá trị cho tất cả các ngày, bạn có thể sử dụng `slide_dbl()`, như đã được trình bày trong chương [Chuỗi thời gian và phát hiện ổ dịch](#timeseries_moving).
...
Dưới đây, chúng tôi trình bày một ví dụ sử dụng `slide_index_dbl()` trong trường hợp linelist. Giả sử rằng mục tiêu của chúng tôi là tính toán tỷ lệ mắc liên tục trong 7 ngày - tính tổng các trường hợp bằng cách sử dụng cửa sổ 7 ngày liên tục. Nếu bạn đang tìm kiếm ví dụ về trung bình động, hãy xem phần bên dưới về [biến động theo nhóm](#roll_slider_group).

Để bắt đầu, bộ dữ liệu `daily_counts` được tạo ra để phản ánh số lượng ca mắc hàng ngày từ `linelist`, như đã được tính toán với `count()` với **dplyr**.
...
Đây là data frame `daily_counts` - có những hàng `nrow(daily_counts)`, mỗi hàng đại diện cho một ngày, nhưng đặc biệt trong giai đoạn đầu của dịch, *có một số ngày không xuất hiện (không có ca mắc nào được tiếp nhận vào những ngày đó)*.

```{r, echo=F}
DT::datatable(daily_counts, rownames = FALSE, options = list(pageLength = 6, scrollX=T) )
```

Điều quan trọng là phải nhận ra rằng một hàm động tiêu chuẩn (như `slide_dbl()` sẽ sử dụng cửa sổ của 7 *hàng*, không phải 7 *ngày*. Vì vậy, nếu có bất kỳ ngày nào trống, một số cửa sổ sẽ thực sự trở nên dài hơn so với 7 ngày theo lịch!

Một cửa sổ động "thông minh" có thể được tạo với `slide_index_dbl()`. "Chỉ mục" có nghĩa là hàm sử dụng một *cột riêng biệt* làm "chỉ mục" cho cửa sổ động. Cửa sổ đó không chỉ đơn giản dựa trên các hàng của data frame.
...
Hãy đưa ra một so sánh. Dưới đây, chúng tôi tính toán tỷ lệ ca mắc động trong 7 ngày với các cửa sổ thông thường và được lập chỉ mục.
...
Mỗi khi một nhóm mới được tạo, cửa sổ trượt sẽ bắt đầu lại. Do đó, một điều cần lưu ý là nếu dữ liệu của bạn được nhóm lại và bạn đã thiết lập `.complete = TRUE`, bạn sẽ có các giá trị trống ở mỗi lần chuyển đổi giữa các nhóm. Khi hàm di chuyển xuống dưới qua các hàng, mọi chuyển đổi trong cột nhóm sẽ bắt đầu lại việc tích lũy kích thước cửa sổ tối thiểu để cho phép tính toán.
...
Bây giờ chúng ta có thể vẽ các đường trung bình động, hiển thị dữ liệu theo nhóm bằng cách chỉ định `~ hospital` thành `facet_wrap()` trong `ggplot()`. Để giải trí, chúng tôi vẽ hai biểu đồ dạng hình học - một `geom_col()` thể hiện số lượng ca mắc hàng ngày và một `geom_col()` thể hiện trung bình động của 7 ngày.
...
[***THẬN TRỌNG:*** Nếu bạn gặp lỗi cho biết *"slide() was deprecated in tsibble 0.9.0 and is now defunct. Please use slider::slide() instead."*, điều đó có nghĩa là hàm `slide()` từ package **tsibble** đang đè lên hàm `slide()` từ package **slider**. Khắc phục lỗi này bằng cách xác định package trong lệnh, ví dụ `slider::slide_dbl()`.]{style="color: red;"}
...
Package **tidyquant** cung cấp một cách tiếp cận khác để tính toán đường trung bình động - lần này là từ *trong* chính lệnh `ggplot()`.

Dữ liệu `linelist` dưới đây được đếm theo ngày khởi phát và được vẽ dưới dạng một đường mờ (`alpha` \<1). Đường trên cùng là một đường được tạo bằng `geom_ma()` từ package **tidyquant**, với cửa sổ được thiết lập là 7 ngày (`n = 7`) với màu sắc và độ dày được chỉ định.
...
-   "EMA" - đường trung bình động lũy thừa (exponential moving average) (giá trị hơn cho các quan sát mới)\
-   "WMA" - đường trung bình động tỉ trọng tuyến tính (weighted moving average) (`wts` được sử dụng để đánh giá tần suất các quan sát trong đường trung bình động)\
-   Các loại đường trung bình động khác có thể được tìm thấy trong tài liệu về các hàm
...
Xem [đặc trưng](https://cran.r-project.org/web/packages/tidyquant/vignettes/TQ04-charting-with-tidyquant.html) này để biết thêm chi tiết về các tùy chọn sẵn có trong **tidyquant**.
