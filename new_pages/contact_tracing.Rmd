...
Chúng tôi sẽ nhập bộ dữ liệu mẫu về những người tiếp xúc và thông tin "theo dõi" của họ. Những dữ liệu này đã được truy xuất và không được lồng ghép với Go.Data API, đồng thời được lưu trữ dưới dạng tệp ".rds".
...
[Bấm để tải xuống dữ liệu đăng ký tiếp xúc (file .rds)](https://github.com/WorldHealthOrganization/godata/blob/master/analytics/r-reporting/data/contacts_clean.rds?raw=true'%20class='download-button)
...
Dữ liệu ở dạng gốc trong các tệp có thể được tải xuống, dữ liệu dùng để đối chiếu với dữ liệu gốc được cung cấp bởi Go.Data API (tìm hiểu thêm về [APIs ở đây](#import_api)). Đối với mục đích ví dụ ở đây, chúng tôi sẽ làm sạch dữ liệu để giúp bạn đọc chương này dễ dàng hơn. Nếu bạn đang sử dụng một phiên bản Go.Data, bạn có thể xem hướng dẫn đầy đủ về cách truy xuất dữ liệu của mình [tại đây](https://github.com/WorldHealthOrganization/godata/tree/master/analytics/r-reporting).

Dưới đây, các bộ dữ liệu được nhập bằng hàm `import()` từ package **rio**. Xem chương về [Nhập xuất dữ liệu] để biết những cách nhập dữ liệu khác nhau. Chúng tôi sử dụng `here()` để xác định đường dẫn tệp - bạn nên cung cấp đường dẫn tệp cụ thể cho máy tính của mình. Sau đó, chúng tôi sử dụng `select()` để chỉ chọn một số cột nhất định của dữ liệu, nhằm đơn giản hóa cho mục đích diễn giải.

#### Dữ liệu ca mắc {.unnumbered}

Những dữ liệu này là một bảng các ca mắc và thông tin về họ.
...
Dưới đây là các trường hợp `nrow(cases)`:

```{r, message=FALSE, echo=F}
DT::datatable(cases, rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```

#### Dữ liệu người tiếp xúc {.unnumbered}

Những dữ liệu này là một bảng gồm tất cả những người tiếp xúc và thông tin về họ. Một lần nữa, cung cấp đường dẫn tệp của riêng bạn. Sau khi nhập, chúng tôi thực hiện một số bước làm sạch dữ liệu sơ bộ bao gồm:

-   Thiết lập age_class như một hệ số (factor) và đảo ngược thứ tự hàng sao cho những người trẻ hơn nằm ở trên đầu\
-   Chỉ chọn một số cột nhất định, trong khi đặt tên lại cho một trong số chúng\
-   Giả sử các hàng được chỉ định có cấp quản trị viên là 2 bị missing cho "Djembe", để cải thiện độ rõ ràng của một số hình ảnh trực quan mẫu
...
Đây là các hàng `nrow(contacts)` của bộ dữ liệu `contacts`:
...
Chúng tôi nhập dữ liệu và thực hiện một số bước làm sạch. Chúng tôi chọn một số cột nhất định và cũng chuyển đổi trong một cột ký tự tất cả các giá trị thành chữ thường.
...
Đây là 50 hàng đầu tiên của `nrow(followups)` - hàng của bộ dữ liệu `followups` (mỗi hàng là một người tiếp xúc tiếp theo, với trạng thái kết quả trong cột `followup_status`):
...
Ở đây chúng tôi nhập dữ liệu cho thấy mối quan hệ giữa các ca mắc và người tiếp xúc. Chúng tôi chọn cột nhất định để hiển thị.
...
Kim tự tháp dưới đây so sánh sự phân bố theo độ tuổi của những người tiếp xúc, theo giới tính. Lưu ý rằng những người tiếp xúc bị missing tuổi được đưa vào thanh riêng của họ ở trên cùng. Bạn có thể thay đổi hành động mặc định này, nhưng sau đó hãy xem xét liệt kê số bị missing trong chú thích.
...
Bây giờ chúng ta có thể vẽ biểu đồ bộ dữ liệu đã chuyển đổi này với `age_pyramid()` như trước đây, nhưng thay thế `gender` với `category` (người tiếp xúc, hoặc ca mắc).
...
Chúng tôi bắt đầu bằng cách đếm số hàng (người tiếp xúc) trên mỗi ca mắc nguồn. Kết quả này được lưu dưới dạng một data frame.
...
Chúng tôi sử dụng `geom_histogram()` để vẽ các dữ liệu này dưới dạng biểu đồ tần suất.
...
Trong ví dụ Go.Data của chúng tôi, những dữ liệu này được lưu trữ trong data frame `followups`, data frame này có định dạng "dọc" với một hàng cho mỗi tương tác theo dõi. 50 hàng đầu tiên nhìn như sau:
...
[***CHÚ Ý:*** Cẩn thận với các bản trùng lặp khi xử lý dữ liệu theo dõi; vì có thể có một vài lần theo dõi sai sót trong cùng một ngày cho một người tiếp xúc nhất định. Nó dường như trông có vẻ là một lỗi nhưng lại phản ánh thực tế - ví dụ: người theo dõi trường hợp tiếp xúc có thể gửi biểu mẫu theo dõi từ sớm trong ngày dù họ chưa liên hệ được với người tiếp xúc và gửi biểu mẫu thứ hai khi họ đã liên hệ được sau đó. Việc này sẽ phụ thuộc vào quy trình thực hiện hoạt động đối với cách bạn muốn xử lý các bản trùng lặp - chỉ cần đảm bảo ghi lại cách tiếp cận của bạn một cách rõ ràng.]{style="color: orange;"}
...
Trong dữ liệu ví dụ của chúng tôi, các bản ghi duy nhất áp dụng điều này là những bản ghi missing ID! Chúng tôi có thể loại bỏ chúng. Tuy nhiên, với mục đích diễn giải, chúng tôi sẽ trình bày các bước nhằm loại bỏ trùng lặp để mỗi người tiếp xúc chỉ có một lần theo dõi truy vết mỗi ngày. Xem chương [Loại bỏ trùng lặp] để biết thêm chi tiết. Chúng tôi sẽ giả định rằng bản ghi cuộc gọi truy vết mới nhất là bản ghi chính xác. Chúng tôi cũng tận dụng cơ hội để làm sạch cột `followup_number` ("ngày" theo dõi sẽ nằm trong khoảng 1 - 14).
...
Đối với mỗi cuộc gọi truy vết tiếp theo, chúng tôi có một trạng thái theo dõi (chẳng hạn như cuộc gọi truy vết có xảy ra hay không và nếu có, người tiếp xúc có triệu chứng hay không). Để xem tất cả các giá trị, chúng ta có thể chạy nhanh lệnh `tabyl()` (từ **janitor**) hoặc `table()` (từ **base** R) (xem chương [Bảng mô tả]) bằng `followup_status` để xem tần suất của từng kết quả.

Trong bộ dữ liệu này, "seen_not_ok" có nghĩa là "được thấy không có triệu chứng" và "seen_ok" có nghĩa là "được thấy có triệu chứng".
...
Vì dữ liệu ngày là liên tục, chúng tôi sẽ sử dụng biểu đồ tần suất để vẽ chúng với `date_of_followup` được gán cho trục x. Chúng tôi có thể vẽ được biểu đồ tần suất "xếp chồng" bằng cách chỉ định đối số `fill =` trong `aes()`, đối số mà chúng tôi gán cho cột `followup_status`. Do đó, bạn có thể thiết lập chú thích bằng cách sử dụng đối số `fill =` của `labs()`.
...
[***CHÚ Ý:*** Nếu bạn đang chuẩn bị nhiều biểu đồ (ví dụ: cho nhiều thành phần pháp lý), bạn sẽ muốn các chú thích xuất hiện giống hệt nhau ngay cả với các mức độ hoàn thiện dữ liệu hoặc thành phần dữ liệu khác nhau. Có thể có những biểu đồ mà không phải tất cả các trạng thái theo dõi đều có trong dữ liệu, nhưng bạn vẫn muốn các danh mục đó xuất hiện trong chú thích. Trong ggplots (như trên), bạn có thể chỉ định đối số `drop = FALSE` của `scale_fill_discrete()`. Trong bảng, hãy sử dụng `tabyl()` để hiển thị số lượng cho tất cả các cấp nhân tố hoặc nếu sử dụng `count()` từ **dplyr**, hãy thêm đối số `.drop = FALSE` để bao gồm số lượng của tất cả các cấp nhân tố.]{style="color: orange;"}
...
Có lẽ những dữ liệu theo dõi này đang được kiểm tra hàng ngày hoặc hàng tuần để đưa ra quyết định hành động. Bạn có thể muốn những phân tách có ý nghĩa hơn theo khu vực địa lý hoặc theo nhóm truy vết tiếp xúc. Chúng ta có thể làm điều này bằng cách điều chỉnh các cột được cung cấp cho `group_by()`.
...
Có nhiều nguồn KPI truy vết tiếp xúc, chẳng hạn như nguồn này từ [ResolveToSaveLives.org](https://contacttracingplaybook.resolvetosavelives.org/checklists/metrics). Phần lớn công việc sẽ là xem xét cấu trúc dữ liệu và suy nghĩ của bạn thông qua tất cả các tiêu chí bao gồm/loại trừ. Chúng tôi đưa ra một vài ví dụ dưới đây; sử dụng cấu trúc siêu dữ liệu Go.Data:
...
Dưới đây, chúng tôi sẽ giới thiệu một bài tập mẫu về cách tạo bảng đẹp mắt để hiển thị thông tin theo dõi người tiếp xúc trên các khu vực quản trị. Cuối cùng, chúng tôi sẽ tạo bảng phù hợp để trình bày với package **formattable** (nhưng bạn có thể sử dụng các package khác như **flextable** - xem chương [Trình bày bảng]).
...
Bây giờ, dựa trên cấu trúc dữ liệu của chúng tôi, chúng tôi sẽ làm như sau:
...
2)  Nối dữ liệu này với dữ liệu người tiếp xúc, dữ liệu mà chứa các thông tin khác như trạng thái tổng thể của người tiếp xúc, ngày tiếp xúc cuối với một ca mắc, v.v. Ngoài ra, chúng tôi sẽ tính toán các số liệu được quan tâm cho mỗi người tiếp xúc, chẳng hạn như số ngày kể từ lần phơi nhiễm cuối\
3)  Chúng tôi nhóm dữ liệu người tiếp xúc nâng cao theo vùng địa lý (`admin_2_name`) và tính toán tóm tắt thống kê cho mỗi khu vực\
4)  Cuối cùng, chúng tôi định dạng bảng thích hợp để trình bày

Đầu tiên, chúng tôi tóm tắt dữ liệu theo dõi để có được thông tin quan tâm:
...
Bây giờ chúng tôi sẽ thêm thông tin này vào bộ dữ liệu `contacts` và tính toán một số cột bổ sung.
...
Tiếp theo, chúng tôi tổng hợp dữ liệu người tiếp xúc theo khu vực, để có một data frame ngắn gọn của các cột thống kê đã tóm tắt.
...
Và bây giờ chúng tôi áp dụng kiểu bảng từ các package **formattable** và **knitr**, bao gồm một chú thích cuối trang hiển thị "kể từ" ngày.
...
Như đã thảo luận trong chương [Biểu đồ nhiệt], bạn có thể tạo ma trận "ai đã lây nhiễm cho ai" bằng cách sử dụng `geom_tile()`.

Khi những ca tiếp xúc mới được tạo, Go.Data lưu trữ thông tin về mối quan hệ này tại điểm cuối API `relationships`; chúng ta có thể thấy 50 hàng đầu tiên của bộ dữ liệu này bên dưới. Điều này có nghĩa là chúng ta có thể tạo một biểu đồ nhiệt với vài bước tương đối với mỗi tiếp điểm được nối với ca mắc nguồn của nó.
...
Như đã mô tả trước đây, chúng tôi tạo bảng chéo;
