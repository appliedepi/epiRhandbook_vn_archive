...
1) Bạn phải có quyền truy cập quản trị cho máy tính của mình. Thiết lập RStudio đặc biệt để chạy với tư cách quản trị viên. 
...
3) Package **rmarkdown** **không được** lưu trong thư viện package "\\\", vì khi đó nó không thể kết nối với TinyTex hoặc Pandoc. 
...
Bạn cần nhấp vào biểu tượng RStudio để mở RStudio, sau đó bấm chuột phải. Tùy thuộc vào máy của bạn, bạn có thể thấy tùy chọn "Chạy với tư cách Quản trị viên". Nếu không, bạn có thể thấy một tùy chọn để chọn Thuộc tính (sau đó sẽ xuất hiện một cửa sổ có tùy chọn "Tương thích" và bạn có thể chọn hộp kiểm "Chạy với tư cách Quản trị viên"). 
...
Nếu bạn gặp khó khăn với R Markdown kết nối với Pandoc, hãy bắt đầu với đoạn code mã này để tìm hiểu RStudio cho rằng cài đặt Pandoc của bạn ở đâu. 
...
Nếu bạn muốn xem package đang tải từ thư viện nào, hãy thử đoạn code dưới đây: 
...
**"Không phiên dịch được ...nội dung trong rmarkdown"**  
...
**Không thể tải dữ liệu từ mạng internet**  

Ví dụ, `Lỗi trong công cụ::startDynamicHelp() : Không thể tải dữ liệu từ mạng internet`  

* Thử chọn phiên bản 32-bit từ RStudio qua Công cụ/Tùy chọn chung .  
  * Chú ý: nếu phiên bản 32-bit không xuất hiện trong menu, hãy đảm bảo rằng bạn không sử dụng RStudio version 1.2.  
* Ngoài ra, hãy thử gỡ cài đặt R và cài đặt lại với phiên bản bit khác (32 thay vì 64) 


**thư viện trong ổ C không xuất hiện dưới dạng tùy chọn khi cố gắng cài đặt các backage theo cách thủ công**
...
Nếu bạn gặp phải "lỗi pandoc 1" khi knitting tập lệnh R Markdowns trên ổ đĩa mạng:

* Trong số nhiều vị trí thư viện đặt lưu trữ, hãy chọn vị trí các ổ cứng của máy được liệt kê đầu tiên (xem lại ở trên)
* Cso thể làm được giải pháp trên khi knitting trên ổ đĩa cá nhân trong khi máy tính có kết nối internet
* Xem thêm các kỹ thuật tại đây: https://ciser.cornell.edu/rmarkdown-knit-to-html-word-pdf/ 

**Lỗi Pandoc 83**  

Lỗi sẽ có dạng như sau: `can't find file...rmarkdown...lua...`. Điều này có nghĩa là nó không thể tìm thấy tệp rmarkdown này. 
...
1) Các package sử dụng trong file Rmarkdown chưa được cài đặt
2) Không thể tìm thấy gói các package sử dụng trong file Rmarkdown
...
* Ngoài ra, hãy xem package cụ thể nào không thể truy cập được có thể được chuyển đến ổ C: hay không. 
...
* Xem thêm tại đây https://yihui.org/tinytex/r/#debugging for debugging tips.  
...
**Lập bản đồ các ổ đĩa mạng**

Lập bản đồ ổ đĩa mạng có thể gặp một số rủi ro. Hãy tham khảo ý kiến với bộ phận CNTT của bạn trước khi làm!
...
Làm cách nào để mở một tệp "thông qua một ổ đĩa mạng được lập bản đồ"? 

* Trước tiên, bạn cần biết vị trí ổ mạng mà bạn đang cố gắng truy cập.
* Tiếp theo, trong trình quản lý tệp của Windows, bạn sẽ cần phải nhấp chuột phải vào "Máy tính này" ở ngăn bên phải và chọn "Lập bản đồ ổ đĩa mạng".
* Đi qua đoạn hội thoại để xác định vị trí mạng trước đó dưới dạng ổ đĩa có chữ cái.
* Bây giờ bạn có hai cách để truy cập tệp bạn đang mở.  Hãy nhớ sử dụng đường dẫn của tệp trong các ổ cứng. 

**Lỗi trong cài đặt install.packages()**  
