...
Trong chương này, chúng tôi sẽ minh họa một trường hợp thường gặp, đó là tạo ra một biến phân loại mới.

Lưu ý rằng khi bạn chuyển đổi một cột dạng số thành dạng factor, bạn sẽ không thể thực hiện các tính toán thống kê cho dạng số trên cột đó nữa.  

#### Tạo biến {.unnumbered}  

Chúng ta sẽ sử dụng một biến có sẵn, tên là `days_onset_hosp` (số ngày, tính từ khi bắt đầu cho triệu chứng đến khi nhập viện) và tạo một biến mới có tên `delay_cat` bằng cách phân loại các giá trị trong mỗi hàng của biến có sẵn thành một số nhóm khác nhau. Chúng ta sẽ thực hiện việc này bằng hàm `case_when()` trong package **dplyr**, hàm này sẽ giúp áp dụng tuần tự các tiêu chí logic (phía bên phải của dòng lệnh bên dưới) cho mỗi giá trị của biễn có sẵn và trả về giá trị bên trái tương ứng ở biến mới `delay_cat`. Đọc thêm về `case_when()` tại chương [Làm sạch số liệu và các hàm quan trọng].
...
#### Thay đổi thứ tự mặc định cho các giá trị {.unnumbered}  

Khi sử dụng hàm `case_when()` để tạo biến mới, biến mới `delay_cat` là một biến phân loại với các giá trị là ký tự - *chưa* phải là một factor. Do đó, trong bảng tần suất dưới đây, chúng ta thấy rằng các giá trị xuất hiện theo thứ tự mặc định của bảng chữ cái, điều này không có nhiều ý nghĩa trực quan: 

```{r}
table(linelist$delay_cat, useNA = "always")
```

Tương tự như vậy, nếu chúng ta tạo biểu đồ cột, các giá trị cũng xuất hiện theo thứ tự này trên trục x (xem chương [ggplot cơ bản] để hiểu thêm về package **ggplot2** - package giúp trực quan hóa phổ biến nhất trong R).  
...
Hàm `fct_relevel()` có thêm tiện ích cho phép bạn có thể tự sắp xếp trật tự của các thứ bậc trong factor. Đơn giản, bạn chỉ cần viết các thứ bậc theo thứ tự bạn muốn, để chúng trong dấu ngoặc kép, được phân tách bằng dấu phẩy, như được hiển thị bên dưới. Lưu ý rằng chính tả phải khớp chính xác với tên các thứ bậc. Nếu bạn muốn tạo các thứ bậc không tồn tại trong dữ liệu, hãy sử dụng [ hàm`fct_expand()`nhé](#fct_add)). 
...
Bây giờ chúng ta có thể thấy rằng các thứ bậc được sắp xếp, như đã được chỉ định trong lệnh trên, theo một thứ tự hợp lý.

```{r}
levels(linelist$delay_cat)
```

Bây giờ trật tự các cột trong biểu đồ cũng trực quan hơn.s 
...
Lưu ý: Package **forcats** có thể dễ dàng thêm các giá trị missing (`NA`) như là một thứ bậc. Bạn có thể xem thêm tại phần [Giá trị Missing](#fct_missing) dưới đây. 


### Xóa thứ bậc {.unnumbered}  

Nếu bạn sử dụng hàm `fct_drop()`, các thứ bậc "không được sử dụng" và không có biến số nào sẽ bị loại bỏ khỏi factors. Thứ bậc mà chúng ta đã thêm ở trên ("Not admitted to a hospital") có tổn tại nhưng không có biến số nào. Vì vậy, chúng sẽ bị loại bỏ khỏi biến factor của chúng ta bằng cách sử dụng hàm `fct_drop()` như sau:   
...
Package **forcats** cung cấp các chức năng hữu ích để dễ dàng thay đổi trật tự của các thứ bậc trong một biến dạng factor (sau khi một biến số được xác định là một factor): 

Các hàm trong package này có thể được áp dụng cho biến dạng factor trong hai ngữ cảnh dưới đây: 

1) Đối với cột trong một data frame, như thường lệ, việc thay đổi trật tự có thể sử dụng cho bất kỳ phiên làm việc tiếp theo nào
...
Các lệnh trong package **forcats** có thể được sử dụng để thay đổi trật tự các biến trong data frame, hoặc biểu đồ. Bằng cách sử dụng các lệnh "bao quanh" tên của biến *trong* các lệnh vẽ biểu đồ của package `ggplot()`, bạn có thể dảo ngược/thay đổi một trật tự có sẵn của các biến. Sự thay đổi này chỉ áp dụng trong biểu đồ đang vẽ.


Dưới đây, hai biểu đồ đều được vẽ bởi hàm `ggplot()` (xem thêm tại chương [ggplot cơ bản]). Trong biểu đồ đầu tiên, biến `delay_cat` được vẽ trên trục x của biểu đồ với thứ tự các thứ bậc là mặc định trong dữ liệu `linelist`. Trong biểu đồ thứ hai, biến được bao quanh bởi hàm `fct_relevel()` và trật tự của các thứ bậc đã được sắp xếp lại. 
...
Lưu ý rằng, ở biểu đồ thứ hai, tiêu đề mặc định của trục x được hiện khá phức tạp - bạn có thể ghi đè tiêu đề này bằng đối số **ggplot2** `labs()`.  
...
Bạn sẽ thường xuyên cần đảo ngược trật tự của các thứ bậc trong một biến. Đơn giản, bạn chỉ cần thêm hàm `fct_rev()` bao quanh tên factor trong câu lệnh.  
...
Để sắp xếp trật tự các thứ bậc theo tần suất mà nó xuất hiện trong dữ liệu, hãy sử dụng hàm `fct_infreq()`. Bất kỳ thứ bậc nào bị mising (`NA`) sẽ tự động được đưa vào cuối, trừ khi chúng được chuyển sang một thứ bậc khác (xem thêm ở [chương này](#fct_missing)). Bạn có thể đảo ngược trẩt tự bằng cách thêm hàm `fct_rev()` vào câu lệnh. 
...
Sử dụng hàm `fct_inorder()` để thiết lập trật tự mức phù hợp với thứ tự xuất hiện của các biến trong dữ liệu, bắt đầu từ hàng đầu tiên. Điều này có thể hữu ích nếu trước tiên bạn sắp xếp cẩn thận dữ liệu trong data frame bằng hàm `arrange()`, sau đó sử dụng điều này để đặt trật tự các thứ bậc của biến facror. 



### Theo thống kê tóm tắt của một cột khác {.unnumbered}  

Bạn có thể sử dụng hàm `fct_reorder()` để sắp xếp các thứ bậc của một biến *theo thống kê tóm tắt của một biến khác*. Về mặt trực quan, điều này có cho kết quả là các biểu đồ như ý bạn, có các cột/điểm lên hoặc xuống đều đặn trong toàn bộ biểu đồ. 

Trong các ví dụ bên dưới, trục x là `delay_cat`, và trục y là `ct_blood` (giá trị ngưỡng chu kỳ). Biểu đồ Box plots hiển thị phân bố của giá trị CT theo nhóm `delay_cat`. Chúng ta cần sắp xếp các hộp theo thứ tự tăng dần theo giá trị CT trung bình của nhóm. 

Trong ví dụ đầu tiên bên dưới, các thứ bậc được sắp xếp một cách mặc định. Bạn có thể thấy các chiều cao của hộp bị lộn xộn và không theo bất kỳ thứ tự cụ thể nào. Trong ví dụ thứ hai, cột `delay_cat` (được sắp xếp theo trục x) đã được viết lệnh với hàm `fct_reorder()`, cột `ct_blood` được đưa ra làm đối số thứ hai và "trung vị" được đưa ra làm đối số thứ ba (bạn cũng có thể sử dụng "max", "mean", "min", v.v.). Do đó, thứ tự các thứ bậc của biến `delay_cat` giờ sẽ phản ánh các giá trị CT trung bình tăng dần của mỗi giá trị CT trung bình của nhóm `delay_cat`. Điều này được phản ánh trong biểu đồ thứ hai - các hộp đã được sắp xếp lại theo chiều tăng dần. Lưu ý biến missing `NA` sẽ luôn xuất hiện ở cuối, trừ khi được chuyển đổi thành các thứ bậc khác. 
...
Lưu ý trong ví dụ bên trên không có bước nào được yêu cầu trước đó để gọi hàm `ggplot()` - việc nhóm và tính toán đều được thực hiện bên trong lệnh ggplot. 

### Theo giá trị cuối {.unnumbered}  

Sử dụng hàm `fct_reorder2()` cho biểu đồ đường của các nhóm thứ bậc. Hàm sẽ sắp xếp thứ tự các nhóm (bao gồm cả phần *chú giải*) để sắp xếp theo thứ tự dọc của các giá trị ở "cuối" của biểu đồ. Nói về mặt kỹ thuật, nó "sắp xếp theo các giá trị y được kết hợp với các giá trị x lớn nhất." 

Ví dụ, nếu bạn có các dòng hiển thị số lượng trường hợp theo bệnh viện và thời gian, bạn có thể áp dụng hàm `fct_reorder2()` cho đối số `color = ` trong `aes()`, sao cho thứ tự dọc của các bệnh viện xuất hiện trong chú giải phù hợp với thứ tự của các dòng ở cuối của biểu đồ. Đọc thêm trong [tài liệu trực tuyến sau đây](https://forcats.tidyverse.org/reference/fct_reorder.html). 
...
Nếu có giá trị missing `NA` trong biến factor của bạn, bạn có thể dễ dàng chuyển đổi chúng thành một thứ bậc được đặt tên, chẳng hạn như `fct_explicit_na()`. Giá trị missing `NA`  được chuyển đổi thành "(Missing)" sẽ được xếp cuối theo mặc định. Bạn có thể điều chỉnh tên thứ bậc bằng đối số `na_level = `. 

Ví dụ dưới đây, thao tác này được thực hiện trên biến `delay_cat` và một bảng được in bằng` tabyl()`với biến missing `NA` được chuyển thành "Missing delay".
...
Bạn có thể điều chỉnh sự hiển thị của các thứ bậc theo cách thủ công với  `fct_recode()`. Điều này giống như hàm `recode()` trong package **dplyr** (xem thêm tại chương [Làm sạch số liệu và các hàm quan trọng]), nhưng nó cho phép tạo các thứ bậc mới trong factor. Nếu bạn sử dụng đơn giản `recode()` trên một factor, các giá trị được mã hóa mới sẽ bị từ chối trừ khi chúng đã được đặt ở thứ bậc cho phép. 

Công cụ này cũng có thể được sử dụng để "kết hợp" các thứ bậc trong factor, bằng cách gán cho nhiều thứ bậc cùng một giá trị được mã hóa lại. Chỉ cần cẩn thận để không bị mất thông tin! Cân nhắc thực hiện các bước kết hợp này trong một biến mới (không ghi đè lên biến hiện có). 
...
### Chuyển thành biến "Other" {.unnumbered}  

Bạn có thể sử dụng hàm`fct_other()` để gán các thứ bậc của factor theo cách thủ công cho mức “Other”. Dưới đây, tất cả các thứ bậc trong biến `hospital`, ngoài "Port Hospital" và "Central Hospital", được gộp chung thành “Other”. Bạn có thể cung cấp một vectơ để giữ `keep = `, hoặc thả `drop = `. Bạn có thể thay đổi cách hiển thị của thứ bậc bằng hàm `other_level = `.
...
### Giảm theo tần suất {.unnumbered}

Bạn có thể tự động kết hợp các thứ bậc trong biến factor ít thường xuyên nhất bằng cách sử dụng hàm `fct_lump()`.

Để "gộp" nhiều mức tần số thấp lại thành một nhóm khác "Other", hãy thực hiện một trong các thao tác sau:

* Đặt `n = ` là số nhóm bạn muốn giữ. N cấp độ thường xuyên nhất sẽ được giữ nguyên và tất cả các cấp độ khác sẽ kết hợp thành nhóm "Other". 
* Đặt `prop = ` làm tỷ lệ tần số ngưỡng cho các mức bạn muốn giữ ở trên. Tất cả các giá trị khác sẽ kết hợp thành nhóm "Other".
...
, warn
## Hiển thị tất cả thứ bậc của factor  

Một lợi ích của việc sử dụng factors là chuẩn hóa sự xuất hiện của các chú thích và giảng trong biểu đồ, bất kể giá trị nào thực sự có trong tập dữ liệu.

Nếu bạn đang chuẩn bị nhiều số liệu (ví dụ: cho nhiều khu vực pháp lý), bạn sẽ muốn các chú giải và bảng xuất hiện giống hệt nhau ngay cả với các mức độ hoàn thành dữ liệu hoặc thành phần dữ liệu khác nhau. 

### Trong biểu đồ {.unnumbered}  

Trong hàm vẽ biểu đồ `ggplot ()`, chỉ cần thêm đối số `drop = FALSE` trong hàm liên quan `scale_xxxx() `. Tất cả các thứ bậc trong biến factor sẽ được hiển thị, bất kể chúng có trong dữ liệu hay không. Nếu các thứ bậc trong biến factor của bạn được hiển thị bằng cách sử dụng `fill = `, thì trong scale_fill_discrete (), bạn cần thêm `drop = FALSE`, như được hiển thị bên dưới. Nếu các thứ bậc trong biến factor của bạn được hiển thị với `x = ` (đến trục x) `color = ` hoặc `size = `, bạn sẽ cung cấp điều này cho`scale_color_discrete()` or `scale_size_discrete()` cho phù hợp. 

Ví dụ này là một biểu đồ thành xếp chồng lên nhau theo danh mục tuổi, theo bệnh viện. Việc thêm `scale_fill_discrete (drop = FALSE)` đảm bảo rằng tất cả các nhóm tuổi đều xuất hiện trong chú giải, ngay cả khi không có trong dữ liệu. 
...
Nếu bạn sử dụng hàm `count()` hoặc `summarise()` từ package **dplyr** để vẽ bảng, hãy thêm đối số `.drop = FALSE` để hiển thị số lượng cho tất cả các thứ bậc trong factor ngay cả các thứ bậc không sử dụng.
...
### Tuần dịch tễ với biểu đồ {.unnumbered}  
...
Đầu tiên, sử dụng hàm `format()` từ **base** R to để chuyển đổi hiển thị ngày từ hiển thị YYYY-MM-DD sang hiển thị YYYY-Www (xem thêm tại chương [Làm việc với ngày tháng]). Trong quá trình này, dạng của biến số sẽ được chuyển đổi thành ký tự. Sau đó, chuyển đổi từ dạng ký tự sang dạng Factor với hàm `factor()`.  
...
<span style="color: red;">**Nguy hiểm:_** Nếu bạn đặt các tuần trước các năm ("Www-YYYY") ("%W-%Y"), thứ tự các thứ bậc sẽ sắp xếp mặc định theo bảng chữ cái và điều này là không chính xác (ví dụ: 01-2015 sẽ là trước 35-2014). Bạn có thể cần phải điều chỉnh thứ tự theo cách thủ công, đây sẽ là một quá trình dài khó khăn .</span>  

2) *Để hiển thị nhanh theo mặc định*, sử dụng hàm `date2week()` trong package **aweek**. bạn có thể đặt ngày theo hàm `week_start = `, và nếu bạn đặt đối số `factor = TRUE` thì cột đầu ra là một factor có thứ tự. Như một phần thưởng, factor sẽ bao gồm các thứ bậc cho *tất cả* các tuần có thể có trong khoảng thời gian - ngay cả khi không có trường hợp nào xuất hiện trong tuần đó. 
...
R cho trang Khoa học dữ liệu, chương [factors](https://r4ds.had.co.nz/factors.html)  
