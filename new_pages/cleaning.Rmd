...
* `starts_with()` - khớp với một tiền tố được chỉ định  
  * Ví dụ: `select(starts_with("date_"))`  
* `ends_with()`   - khớp với một hậu tố được chỉ định  
  * Ví dụ: `select(ends_with("_post"))`  
* `matches()`     - áp dụng một biểu thức chính quy (regex)  
  * Ví dụ: `select(matches("[pt]al"))`  
* `num_range()`   - khớp với các cột được đánh số có tiền tố chuẩn, ví dụ: "sym1", "sym2", "sym3"  
* `any_of()`      - khớp với cột NẾU tồn tại nhưng không trả về lỗi nếu không tìm thấy  
  * Ví dụ: `select(any_of(date_onset, date_death, cardiac_arrest))`  

Thêm vào đó, use normal operators such as `c()` to list several columns, `:` for consecutive columns, `!` for opposite, `&` for AND, and `|` for OR.  
