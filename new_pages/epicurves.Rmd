...
Cũng giải quyết các trường hợp cụ thể như:

-   Lập biểu đồ dữ liệu đếm tổng hợp\
-   Các khía cạnh hoặc tạo nhiều cấu phần nhỏ\
-   Áp dụng đường trung bình động\
-   Hiển thị dữ liệu "dự kiến" hoặc có thể bị chậm trễ trong báo cáo\
-   Thêm tỷ lệ ca tích lũy bằng cách sử dụng trục thứ hai
...
Đoạn code này hiển thị cách gọi các package cần thiết cho các phân tích. Trong cuốn sách này, chúng ta nhấn mạnh đến `p_load()` từ **pacman**, sẽ được cài đặt package nếu cần *và* gọi package để sử dụng. Bạn cũng có thể cài đặt package với `library()` từ **base** R. Xem chương [R cơ bản] để biết thêm thông tin về các package trong R.
...
-   Số lượng tổng hợp theo bệnh viện từ cùng một dịch bệnh mô phỏng

Các bộ dữ liệu được nạp bằng hàm `import()` từ package **rio**. Xem chương về [Nhập và xuất dữ liệu] để biết các cách nạp dữ liệu khác nhau.
...
Chúng tôi nạp bộ dữ liệu về các ca bệnh mô phỏng từ một vụ dịch Ebola. Nếu bạn muốn tải xuống dữ liệu để làm theo từng bước, hãy xem hướng dẫn trong chương [Tải sách và dữ liệu]. Chúng tôi giả sử các tệp tin nằm trong thư mục làm việc nên không có thư mục con nào được chỉ định trong đường dẫn này.
...
### Thiết lập các thông số {.unnumbered}
...
Xác minh rằng mỗi cột ngày có liên quan là phân lớp Ngày và có phạm vi giá trị thích hợp. Bạn có thể thực hiện việc này đơn giản bằng cách sử dụng `hist()` cho biểu đồ hoặc `range()` với `na.rm=TRUE`, hoặc với `ggplot()` như bên dưới.
...
Dưới đây, chúng tôi trình bày cách tạo các đường cong dịch bệnh bằng cách sử dụng package **precision2**. Các tác giả của package này đã cố gắng cho phép người dùng tạo và sửa đổi các đường cong dịch bệnh mà không cần biết cú pháp **ggplot2**. Phần lớn nội dung của chương này được điều chỉnh từ package **vignettes**, bạn có thể tìm thấy tại chương **incidence2** [github](https://github.com/reconhub/incidence2).
...
1)  **Tạo** ra một *tỷ lệ mắc bệnh* (sử dụng hàm `incidence()`)

    -   Cung cấp dữ liệu\
    -   Xác định cột ngày tại `date_index =`\
    -   Xác định `interval =` trong các ca nên được tổng hợp (hàng ngày, tuần, tháng..)\
    -   Xác định bất kỳ cột-nhóm nào (ví dụ như giới tính, bệnh viện, kết quả)\

2)  **Vẽ biểu đồ** tỷ lệ mắc bệnh

    -   Xác định nhãn, màu, tiêu đề, ...

Dưới đây, chúng tôi gọi package **incidence2**, tạo tỷ lệ mắc bệnh từ `linelist` trên cột `date_onset` và các trường hợp tổng hợp theo ngày. Sau đó, chúng tôi tổng hợp về tỷ lệ mắc bệnh.
...
Đối tượng **incidence2** trông giống như một tibble (giống như một bộ dữ liệu) và có thể được hiển thị hoặc thao tác thêm như một bộ dữ liệu.
...
Để *vẽ biểu đồ tỷ lệ mắc bệnh*, hãy sử dụng `plot()` với *tên của chỉ số tỷ lệ mắc bệnh*. Ở phía sau, hàm `plot.incidence2()` được gọi, vì vậy để đọc tài liệu cụ thể về t**incidence2**, bạn cần chạy lệnh `?plot.incidence2`.
...
Argument `interval =` của `incidence()` xác định cách các quan sát được nhóm thành các thanh dọc trong biểu đồ.

**Xác định khoảng thời gian**

**incidence2** đem lại sự linh hoạt và cú pháp dễ hiểu để bạn có thể dễ dàng tổng hợp số ca bệnh thành các biểu đồ cột-đường cong dịch bệnh. Cung cấp một giá trị như những giá trị bên dưới cho argument `interval =`. Bạn có thể viết bất kỳ thông tin nào bên dưới dưới dạng số nhiều (ví dụ: "week**s**") và bạn có thể thêm số phía trước (ví dụ: "3 months").

| Tùy chọn argument       | Chú thích                                                 |
|-------------------------|-----------------------------------------------------------|
| Số (1, 7, 13, 14, etc.) | số ngày mỗi khoảng thời gian                              |
| "tuần"                  | ghi chú: mặc định ngày bắt đầu là Thứ 2                   |
| "2 tuần"                | hoặc 3, 4, 5...                                           |
| "tuần Chủ nhật"         | tuần bắt đầu tính từ Chủ nhật (cũng có thể là Thứ 5, ...) |
| "2 tuần Chủ nhật"       | hoặc 3, 4, 5...                                           |
| "tuần MMWR"             | tuần bắt đầu vào Chủ nhật - xem thêm tại CDC Hoa Kỳ       |
| "tháng"                 | ngày đầu của tháng                                        |
| "quí"                   | ngày đầu của tháng của quí                                |
| "2 tháng"               | hoặc 3, 4, 5...                                           |
| "năm"                   | ngày đầu của năm                                          |

Dưới đây là ví dụ về các khoảng thời gian khác nhau trông như thế nào khi áp dụng vào bộ số liệu `linelist`. Lưu ý cách định dạng mặc định và tần suất của các *nhãn* ngày trên trục x thay đổi khi khoảng ngày thay đổi.
...
Bạn có thể tùy chọn chỉ định một giá trị thuộc phân lớp Ngày (ví dụ: `as.Date("2016-05-01")`) đến `firstdate =` trong lệnh `incidence()`. Nếu được cung cấp, dữ liệu sẽ được cắt bớt theo phạm vi này và khoảng thời gian sẽ bắt đầu vào ngày này.

### Nhóm {.unnumbered}

Các nhóm được chỉ định trong lệnh `incidence()` và có thể được sử dụng để tô màu cho các cột hoặc các mặt của dữ liệu. Để chỉ định các nhóm trong dữ liệu của bạn, hãy cung cấp tên cột cho argument `groups =` trong `incidence()` (không có dấu ngoặc kép xung quanh tên cột). Nếu chỉ định nhiều cột, hãy để tên cột trong `c()`.

Bạn có thể chỉ định rằng các trường hợp có giá trị bị thiếu trong các cột nhóm được liệt kê như một nhóm `NA` riêng biệt bằng cách thiết lập `na_as_group = TRUE`. Nếu không, chúng sẽ bị loại khỏi biểu đồ.

-   Để *tô màu các cột theo cột nhóm*, bạn cần cung cấp lại tên cột để `fill =` trong lệnh `plot()`.

-   Về *các khía cạnh dựa trên cột nhóm*, hãy xem chương bên dưới về các khía cạnh của **incidence2**.
...
[***MẸO***: Thay đổi tiêu đề của chú giải bằng cách thêm `+` vào lệnh **ggplot2** `labs(fill = "your title")` vào biểu đồ **incidence2**.]{style="color: darkgreen;"}
...
Bạn có thể đặt argument `na_as_group =` thành FALSE trong lệnh `incidence()` để loại bỏ các hàng có giá trị bị thiếu khỏi biểu đồ.
...
3)  Lập biểu đồ về chỉ số tỷ lệ mắc bệnh

Ví dụ dưới đây sử dụng dữ liệu được lọc để chỉ hiển thị các trường hợp tại Bệnh viện Trung ương.
...
Nếu dữ liệu ban đầu của bạn được tổng hợp (số lượng), hãy cung cấp tên của cột chứa ca đếm trong argument `count =` khi tạo chỉ số tỷ lệ mắc bệnh với hàm `incidence()`.

Ví dụ, bộ dữ liệu `count_data` này là danh sách dòng được tổng hợp thành số lượng hàng ngày theo bệnh viện. 50 hàng đầu tiên hiển thị như sau:

```{r message=FALSE, echo=F}
DT::datatable(head(count_data,50), rownames = FALSE, options = list(pageLength = 5, scrollX=T), class = 'white-space: nowrap' )
```

Nếu bạn đang bắt đầu phân tích với dữ liệu đếm hàng ngày như bộ dữ liệu ở trên, thì lệnh `incidence()` để chuyển đổi dữ liệu này thành đường cong dịch bệnh hàng tuần theo bệnh viện sẽ hiển thị như sau:
...
Để chia nhỏ dữ liệu theo nhóm (ví dụ tạo "small multiples"):

1)  Xác định các cột cần chia nhỏ với `groups =` khi bạn tạo chỉ số tỷ lệ mắc bệnh\
2)  Sử dụng lệnh `facet_plot()` thay cho `plot()`\
3)  Xác định cột nhóm nào được sử dụng cho `fill =` và cho `facets =`

Dưới đây, chúng tôi đặt cả hai cột `hospital` và `outcome` làm nhóm các cột trong lệnh `incidence()`. Sau đó, trong `facet_plot()`, chúng ta vẽ biểu đồ đường cong dịch bệnh, chỉ rõ rằng chúng ta muốn một đường cong dịch bệnh khác nhau cho mỗi bệnh viện và trong mỗi đường cong dịch bệnh, các cột nên được xếp chồng lên nhau và được tô màu theo kết quả.
...
Lưu ý rằng package **ggtree** (được sử dụng để trực quan cây phả hệ) cũng có một hàm là `facet_plot()` - đây là lý do tại sao chúng tôi đã chỉ định `incidence2::facet_plot()` như ở trên.

### Hiệu chỉnh với `plot()` {.unnumbered}

Một đường cong dịch bệnh được tạo ra bởi **incidence2** có thể được sửa đổi thông qua các argument *trong hàm `plot()`*.

**Dưới đây là các arguments trong `plot()` để điều chỉnh cột:**

+----------------+--------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
| Argument       | Mô tả                                                                                                              | Ví dụ                                                               |
+================+====================================================================================================================+=====================================================================+
| `fill =`       | Màu cột. Tên màu hoặc tên cột đã được chỉ định trước đó cho `groups =` trong lệnh `incidence()`                    | `fill = "red"`, hoặc `fill = gender`                                |
+----------------+--------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
| `color =`      | Tô màu xung quanh mỗi cột hoặc xung quanh mỗi nhóm trong một cột                                                   | `border = "white"`                                                  |
+----------------+--------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
| `legend =`     | Vị trí danh mục                                                                                                    | Một trong các lựa chọn "dưới", "trên", "trái", "phải", hoặc "không" |
+----------------+--------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
| `alpha =`      | Độ trong của cột                                                                                                   | 1 là hoàn toàn không trong suốt, 0 là hoàn toàn trong suốt          |
+----------------+--------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+
...
|                |                                                                                                                    |                                                                     |
|                | Sự logic; nếu TRUE, mỗi trường hợp hiển thị như một hộp. Cách hiển thị này là tốt nhất trên các đợt bùng phát nhỏ. |                                                                     |
+----------------+--------------------------------------------------------------------------------------------------------------------+---------------------------------------------------------------------+

**Dưới đây là các argument `plot()` để sửa đổi trục ngày:**

+------------------+--------------------------------------------------------------------------------------------------------------------------------+
| Argument(s)      | Mô tả                                                                                                                          |
+==================+================================================================================================================================+
| `centre_dates =` | TRUE/FALSE về việc hiển thị ngày tháng xuất hiện ở giữa các cột hay ở đầu các cột                                              |
+------------------+--------------------------------------------------------------------------------------------------------------------------------+
...
Lưu ý rằng argument `date_breaks =` chỉ hoạt động nếu `centre_dates = FALSE`. Cung cấp giá trị ký tự trong dấu ngoặc kép bằng cú pháp strptime như bên dưới, cũng như được trình bày chi tiết trong chương [Làm việc với ngày tháng]. Bạn có thể sử dụng `\n` để tạo "dòng mới".

%d = Số ngày của tháng (5, 17, 28, ...)\
%j = Số ngày của năm (ngày Julian 001-366)\
%a = Thứ của tuần viết tắt (Mon, Tue, Wed, ...)\
%A = Thứ của tuần viết đủ (Monday, Tuesday, ...)\
%w = Số thứ tự ngày của tuần (0-6, Chủ nhật là 0)\
%u = Số thứ tự ngày của tuần (1-7, Thứ hai là 1)\
%W = Số tuần (00-53, Thứ hai là ngày bắt đầu)\
%U = Số tuần (01-53, Chủ nhật là ngày bắt đầu)\
%m = Số tháng (Ví dụ 01, 02, 03, 04)\
%b = Tháng viết tắt (Jan, Feb, ...)\
%B = Tháng viết đầy đủ (January, February, ...)\
%y = 2-ký tự năm (Ví dụ 89)\
%Y = 4-ký tự năm (Ví dụ 1989)\
%H = giờ (khung 24 giờ)\
%M = phút\
%S = giây\
%z = bù từ múi giờ GMT\
...
**Dưới đây là các arguments trong `plot()` để hiệu chỉnh các nhãn trong biểu đồ:**
...
+-------------+---------------------------------------------------------------------------------------------------------------------------------------+
| `size =`    | Kích thước của đoạn văn bản trên trục x tính bằng pts (sử dụng hàm `theme()` trong ggplot để điều chỉnh các thông số kích thước khác) |
+-------------+---------------------------------------------------------------------------------------------------------------------------------------+

Một ví dụ sử dụng các argument ở trên:
...
Để điều chỉnh thêm giao diện hiện thị của biểu đồ, hãy xem chương bên dưới về các sửa đổi với `ggplot()`.

### Hiệu chỉnh với ggplot2 {.unnumbered}

Bạn có thể sửa đổi thêm biểu đồ **incidence2** bằng cách thêm các sửa đổi **ggplot2** với dấu `+` sau khi đóng hàm biểu diễn tỷ lệ mắc bệnh `plot()`, như được minh họa bên dưới.

Dưới đây, kết thúc biểu đồ **incidence2** và sau đó các lệnh **ggplot2** được sử dụng để sửa đổi các trục, thêm chú thích và điều chỉnh phông chữ đậm và kích thước chữ.

Lưu ý rằng nếu bạn thêm `scale_x_date()`, hầu hết các định dạng ngày từ `plot()` sẽ bị ghi đè. Xem chương `ggplot()` đường cong dịch bệnh và chương [Các tips với ggplot] để có thêm lựa chọn.
...
Cung cấp tên được định danh trước đó trong bảng màu vào argument `col_pal =` trong `plot()`. Package **incidence2** đi kèm với 2 bảng màu được xác định trước: "vibrant" và "muted". Trong "vibrant", có 6 màu đầu tiên là riêng biệt và trong "muted" có 9 màu đầu tiên là khác biệt. Sau những con số này, các màu là thêm/trung gian của các màu khác. Bạn có thể tìm thấy những bảng màu được xác định trước này tại [website này](https://personal.sron.nl/~pault/#sec:qualitative). Các bảng màu trừ màu xám, được dành riêng cho dữ liệu bị thiếu (sử dụng `na_color =` để thay đổi mặc định này).
...
Để chỉ định màu theo cách thủ công, hãy thêm hàm **ggplot2** `scale_fill_manual()` với hàm `plot()` bằng dấu `+` và cung cấp vectơ tên màu hoặc HEX code cho các giá trị argument `values =`. Số lượng màu được liệt kê phải bằng số nhóm. Hãy lưu ý, các giá trị bị thiếu có phải là một nhóm hay không - chúng có thể được chuyển đổi thành giá trị dạng ký tự như "Missing" trong quá trình chuẩn bị dữ liệu của bạn bằng hàm `fct_explicit_na()` như được giải thích trong chương [Factors].
...
Như đã đề cập trong chương [Các tips với ggplot], bạn có thể tạo bảng màu của riêng mình bằng cách sử dụng `colorRampPalette()` trên một vectơ màu và chỉ định số màu bạn muốn đổi lại. Đây là một cách tốt để có thể tập hợp được nhiều màu trong bằng cách chỉ định một vài màu.
...
### Điều chỉnh mức độ yêu cầu {.unnumbered}

Để điều chỉnh thứ tự xuất hiện của nhóm (trên biểu đồ và trong chú giải), cột nhóm cần phải là phân lớp phân loại. Xem chương [Factors] để biết thêm thông tin.
...
Bây giờ, để điều chỉnh thứ tự sao cho "Missing" và "Other" ở trên cùng của đường cong dịch bệnh, chúng ta có thể làm như sau:

-   Gọi package **forcats**, để làm việc với cột định dạng phân loại\

-   Điều chỉnh bộ dữ liệu - trong trường hợp này, chúng tôi sẽ xác định một bộ dữ liệu mới (`plot_data`) trong đó:

    -   Cột `gender` được xác định như một biến phân loại thứ tự các cấp được thiết lập bằng `fct_relevel()` để "Other" và "Missing" đứng đầu tiên, vì vậy chúng xuất hiện ở đầu các cột\

-   Tỷ lệ mắc bệnh được tạo và vẽ biểu đồ như trước đó\

-   Chúng ta điều chỉnh thêm với **ggplot2**
...
Nếu bạn vẽ biểu đồ với các thiết lập mặc định của **incidence2**, bạn có thể nhận thấy rằng các đường lưới dọc xuất hiện ở mỗi nhãn ngày và một lần giữa mỗi nhãn ngày. Điều này có thể dẫn đến các đường lưới giao nhau với đỉnh của một số cột.
...
Tuy nhiên, lưu ý rằng nếu sử dụng tuần, các arguments `date_breaks` và `date_minor_breaks` chỉ hoạt động cho các tuần bắt đầu vào *thứ Hai*. Nếu các tuần của bạn diễn ra vào một ngày khác trong tuần, thay vào đó, bạn sẽ cần cung cấp vectơ ngày cho các arguments `breaks =` và `minor_breaks =` theo cách thủ công. Xem chương **ggplot2** để biết các ví dụ về điều này bằng cách sử dụng `seq.Date()`.

### Tỷ lệ tích lũy {.unnumbered}

Bạn có thể dễ dàng tạo ra một biểu đồ của tỷ lệ tích lũy bằng cách chuyển tỷ lệ này đến lệnh **incidence2** `cumulate()` và sau đó đến `plot()`. Điều này cũng tương tự với `facet_plot()`.
...
Xem chương sau để biết phương pháp thay thế để vẽ biểu đồ tỷ lệ tích lũy với **ggplot2** - ví dụ: để chồng một đường tỷ lệ tích lũy lên trên đường cong dịch bệnh.

### Trung bình động {.unnumbered}

Bạn có thể thêm trung bình động vào biểu đồ **incidence2** một cách dễ dàng bằng `add_rolling_average()` từ package **i2extras**. Chuyển incidence2 tới hàm này, rồi đến `plot()`. Đặt `before =` là số ngày trước đó bạn muốn đưa vào trung bình động (mặc định là 2). Nếu dữ liệu của bạn được nhóm lại, trung bình động sẽ được tính cho mỗi nhóm.
...
Sử dụng `ggplot()` để xây dựng đường cong dịch bệnh cho phép linh hoạt và tùy chỉnh hơn, nhưng đòi hỏi nhiều nỗ lực và hiểu biết hơn về sử dụng `ggplot()`.

Không giống như sử dụng package **incidence2**, bạn phải kiểm soát *thủ công* việc tổng hợp các trường hợp theo thời gian (theo tuần, tháng, v.v.) *và* khoảng thời gian của các nhãn trên trục ngày. Điều này phải được quản lý rất thận trọng.

Các ví dụ này sử dụng một tập con của bộ dữ liệu `linelist` - chỉ các trường hợp từ Bệnh viện Trung ương.
...
-   Biểu đồ, với các ca bệnh `linelist` được tổng hợp thành "thùng" được phân biệt bằng các điểm "ngắt" cụ thể\
-   Phạm vi các trục và nhãn\
-   Chủ đề của biểu đồ, bao gồm tiêu đề, nhãn, chú thích, v.v.

### Xác định số liệu với biểu đồ "cột" {.unnumbered}

Ở đây chúng ta trình bày cách xa định cách các ca bệnh sẽ được tổng hợp vào các biểu đồ dạng thùng ("cột"). Điều quan trọng cần nhận ra là việc tổng hợp số ca bệnh vào biểu đồ cột không nhất thiết phải cùng khoảng thời gian với ngày sẽ xuất hiện trên trục x.

Dưới đây có lẽ là đoạn code đơn giản nhất để tạo ra các đường cong dịch bệnh hàng ngày và hàng tuần.

Trong lệnh tổng quát `ggplot()`, bộ dữ liệu được đưa vào `data =`. Trên nền tảng này, dạng hình học của biểu đồ được thêm dấu `+`. Trong `geom_histogram()`, chúng tôi ánh xạ tính trục sao cho cột `date_onset` được ánh xạ tới trục x. Cũng trong `geom_histogram()` nhưng *không* phải trong `aes()`, chúng tôi đặt `binwidth =` trong các biểu đồ cột, tính bằng ngày. Nếu cú pháp **ggplot2** này khó hiểu, hãy xem lại chương về [ggplot cơ bản].

[***LƯU Ý***: Lập biểu đồ các trường hợp hàng tuần bằng cách sử dụng `binwidth = 7` bắt đầu cột 7 ngày đầu tiên ở ca bệnh đầu tiên, có thể là bất kỳ ngày nào trong tuần! Để tạo các tuần cụ thể, hãy xem phần bên dưới.]{style="color: orange;"}
...
Chúng tôi xin lưu ý rằng ca bệnh đầu tiên trong bộ dữ liệu của Bệnh viện Trung ương có triệu chứng khởi phát trên:
...
Tạo vectơ ngày tháng với hàm **base** R `seq.Date()`. Hàm này sử dụng các argument `to =`, `from =`, và `by =`. Ví dụ: lệnh bên dưới trả về ngày mỗi tháng bắt đầu từ ngày 15 tháng 1 và kết thúc trước ngày 28 tháng 6.
...
Vectơ này có thể sử dụng trong `geom_histogram()` như `breaks =`:
...
-   Giá trị "from" (ngày sớm nhất của chuỗi) được tạo như sau: giá trị ngày nhỏ nhất (`min()` với `na.rm=TRUE`) trong cột `date_onset` được đưa vào `floor_date()` từ package **lubridate**. `floor_date()` được đặt thành "tuần" trả về ngày bắt đầu của "tuần" của trường hợp đó, với điều kiện là ngày bắt đầu của mỗi tuần là Thứ Hai (`week_start = 1`).\
...
-   Argument "by" của `seq.Date()` có thể được đặt thành bất kỳ số ngày, tuần hoặc tháng nào.\
-   Sử dụng `week_start = 7` cho tuần bắt đầu vào Chủ nhật

Vì chúng ta sẽ sử dụng các vectơ ngày dạng này trong toàn bộ chương này, chúng ta cũng xác định một vectơ cho toàn bộ đợt bùng phát (ở trên chỉ dành cho Bệnh viện Trung ương).
...
Các kết quả trả về của `seq.Date()` có thể được sử dụng để tạo đoạn ngắt trong biểu đồ, nhưng cũng có thể ngắt cho các nhãn ngày, có thể độc lập với các cột. Đọc thêm về nhãn ngày trong các chương sau.

[***MẸO***: Đối với lệnh `ggplot()` đơn giản hơn, hãy lưu các đoạn ngắt cột và ngắt nhãn ngày dưới dạng các vectơ đã đặt tên trước và gán tên này vào `breaks =`.]{style="color: darkgreen;"}

### Ví dụ về đường cong dịch bệnh theo tuần {.unnumbered}

**Dưới đây là code ví dụ chi tiết để tạo đường cong dịch bệnh hàng tuần cho các tuần bắt đầu vào Thứ Hai, với các cột căn chỉnh, nhãn ngày và đường lưới dọc**. Chương này dành cho người dùng cần code nhanh. Để hiểu sâu từng khía cạnh (chủ đề, nhãn ngày, v.v.), hãy tiếp tục xem các chương tiếp theo. Chú ý:

-   Các *đoạn chia biểu đồ* được xác định bằng `seq.Date()` như được giải thích ở trên để bắt đầu vào thứ Hai trước ca sớm nhất và kết thúc vào thứ Hai sau ca cuối cùng\
-   Khoảng *nhãn ngày* được xác định bởi `date_breaks =` trong `scale_x_date()`\
-   Khoảng đường lưới dọc nhỏ giữa các nhãn ngày được xác định bởi `date_minor_breaks =`\
-   `expand = c(0,0)` trong trực x và y loại bỏ không gian thừa trên mỗi cạnh của trục, điều này cũng đảm bảo các nhãn ngày bắt đầu từ cột đầu tiên.
...
-   Các điểm ngắt của các *biểu đồ cột* phải được đặt thành Chủ nhật (`week_start = 7`)\
...
Ví dụ: lệnh `scale_x_date()` cho các tuần bắt đầu vào Chủ nhật có thể hiển thị như sau:
...
Các cột biểu đồ có thể được tô màu theo nhóm và "xếp chồng lên nhau". Để chỉ định cột nhóm, hãy thực hiện các thay đổi sau. Xem trang [ggplot cơ bản] để biết thêm chi tiết.

-   Trong hàm ánh xạ trục của biểu đồ `aes()`, ánh xạ tên cột với argument `group =` và `fill =`\
-   Loại bỏ bất kỳ argument `fill =` nào bên ngoài `aes()`, vì nó sẽ ghi đè argument bên trong\
-   Argument *bên trong* `aes()` sẽ áp dụng *theo nhóm*, trong khi bất kỳ argument bên ngoài nào đều áp dụng cho tất cả các cột (ví dụ: bạn có thể vẫn muốn `color =` bên ngoài, vì vậy mỗi cột có cùng một đường viền)
...
-   Để thiết lập *thủ công* màu cho từng nhóm, hãy sử dụng `scale_fill_manual()` (lưu ý: `scale_fill_manual()` là khác nhau!)

    -   Sử dụng argument `values =` để áp dụng một vectơ màu\
    -   Sử dụng `na.value =` để xác định màu cho giá trị `NA`\
    -   Sử dụng argument `labels =` để thay đổi ký tự văn bản trong mục chú thích. Để cho an toàn, sử dụng như một vectơ tên như `c("old" = "new", "old" = "new")` hoặc điều chỉnh giá trị trong bộ dữ liệu\
...
### Hiệu chỉnh thứ tự cấp độ {.unnumbered}

Thứ tự mà các cột được nhóm xếp chồng lên nhau được điều chỉnh tốt nhất bằng cách phân loại cột nhóm dưới phân lớp phân loại. Sau đó, bạn có thể chỉ định thứ tự cấp độ phân loại (và nhãn hiển thị của chúng). Xem chương về [Factors] hoặc [Các tips với ggplot] để biết thêm chi tiết.

Trước khi tạo biểu đồ, hãy sử dụng hàm `fct_relevel()` từ package **forcats** để chuyển đổi cột nhóm thành phân lớp phân loại và điều chỉnh thứ tự cấp độ theo cách thủ công, như được trình bày chi tiết trong chương về [Factors].
...
Đọc thêm về chú thích và thang đo trong chương [Các tips với ggplot]. Dưới đây là một vài điểm nổi bật:

-   Chỉnh sửa tiêu đề chú thích trong hàm tỷ lệ hoặc với `labs(fill = "Legend title")` (nếu bạn đang sử dụng `color =`, thì hãy sử dụng `labs(color = "")`)\

-   `theme(legend.title = element_blank())` để bỏ trống tiêu đề chú thích\

-   `theme(legend.position = "top")` ("dưới", "trái", "phải", hoặc "không" để bỏ chú thích)
...
Nếu có nhiều hơn hai nhóm giá trị, có thể gây khó đọc. Thay vào đó, hãy cân nhắc sử dụng một biểu đồ nhiều mặt (hệ số nhân nhỏ). Để dễ xem trong ví dụ này, các giá trị giới tính bị thiếu sẽ bị xóa.
...
Ngoài ra, bạn có thể thiết lập giá trị ngày tối đa và tối thiểu bằng cách sử dụng `limits = c()` trong `scale_x_date()`. Ví dụ:
...
### Nhãn trục/đường lưới của ngày {.unnumbered}
...
-   **Nếu cột biểu đồ là ngày, tuần bắt đầu vào thứ hai, tháng hoặc năm:**

    -   Sử dụng `date_breaks =` để xác định khoảng thời gian của các nhãn và đường lưới chính (ví dụ: "ngày", "tuần", "3 tuần", "tháng" hoặc "năm")
...
-   **Nếu cột biểu đồ là các tuần bắt đầu vào Chủ nhật:**
...
-   Xem phần đầu chương ggplot về hướng dẫn cách tạo chuỗi ngày bằng `seq.Date()`.\
-   Xem [chương này](https://rdrr.io/r/base/strptime.html) hoặc chương [Làm việc với ngày tháng] để biết thêm mẹo tạo nhãn ngày.

#### Minh chứng {.unnumbered}

Dưới đây là minh họa biểu đồ trong đó các cột và nhãn biểu đồ/đường lưới được căn chỉnh và không thẳng hàng:
...
### Tổng hợp dữ liệu {.unnumbered}

Thông thường, thay vì bắt đầu với bộ số liệu `linelist`, bạn có thể bắt đầu với số lượng tổng hợp từ các cơ sở y tế, quận, huyện, v.v. Bạn có thể tạo đường cong dịch bệnh với `ggplot()` nhưng code sẽ hơi khác một chút. Chương này sẽ tận dụng bộ dữ liệu `count_data` đã được nạp trước đó, trong chương chuẩn bị dữ liệu. Bộ dữ liệu này là `linelist` được tổng hợp thành số lượng bệnh viện theo ngày. 50 hàng đầu tiên được hiển thị dưới đây.
...
Chúng tôi có thể vẽ biểu đồ đường cong dịch bệnh từ *dữ liệu đếm hàng ngày*. Dưới đây là sự khác biệt trong đoạn code:

-   Trong ánh xạ trục `aes()`, xác định `y =` làm cột đếm (trong trường hợp này, tên cột là `n_cases`)

-   Thêm argument `stat = "identity"` trong `geom_histogram()`, xác định rằng chiều cao cột phải là giá trị `y =`, không phải số hàng như mặc định\

-   Thêm argument `width =` để tránh các đường trắng dọc giữa các cột. Đối với dữ liệu hàng ngày được đặt thành 1. Đối với dữ liệu đếm hàng tuần được đặt thành 7. Đối với dữ liệu đếm hàng tháng, các đường màu trắng là một vấn đề (mỗi tháng có số ngày khác nhau) - hãy xem xét chuyển đổi trục x của bạn thành cột phân loại thứ tự (tháng) và sử dụng `geom_col()`.
...
Bây giờ vẽ biểu đồ sao cho `x =` cột epiweek. Hãy nhớ thêm `y =` cột đếm vào ánh xạ trục và thêm `stat = "identity"` như đã giải thích ở trên.
...
### Đường trung bình động {.unnumbered data-link="Moving averages"}
...
1)  Tổng hợp dữ liệu thành số lượng khi cần thiết (hàng ngày, hàng tuần, v.v.) (xem chương [Nhóm dữ liệu])\
2)  Tạo một cột mới để giữ đường trung bình động, được tạo bằng `slide_index()` từ package **slider**\
3)  Vẽ đường trung bình động dưới dạng `geom_line()` ở phía trên (sau) biểu đồ đường cong dịch bệnh

Tham khảo thêm tại [vignette cho package **slider**](https://cran.r-project.org/web/packages/slider/vignettes/slider.html)
...
Như với các ggplots khác, bạn có thể tạo các biểu đồ được chia nhỏ ("small multiples"). Như đã giải thích trong chương [Các tips với ggplot] trong cuốn sách này, bạn có thể sử dụng `facet_wrap()` hoặc `facet_grid()`. Ở đây chúng tôi chứng minh với `facet_wrap()`. Đối với đường cong dịch bệnh, sử dụng `facet_wrap()` thường dễ dàng hơn vì có khả năng bạn chỉ cần chia nhỏ biểu đồ trong một cột.

Cú pháp chung là `facet_wrap(rows ~ cols)`, trong đó bên trái dấu ngã (\~) là tên của một cột nằm trên các "hàng" của biểu đồ chia nhỏ và ở bên phải dấu ngã là tên của một cột nằm trên các "cột" của biểu đồ chia nhỏ. Đơn giản nhất, chỉ cần sử dụng một tên cột, ở bên phải dấu ngã: `facet_wrap(~age_cat)`.

**Trục tự do**\
Bạn sẽ cần phải quyết định xem tỷ lệ của các trục cho mỗi khía cạnh là "cố định" với cùng một kích thước (mặc định) hay "tự do" (nghĩa là chúng sẽ thay đổi dựa trên dữ liệu với từng khía cạnh). Thực hiện điều này với argument `scales =` trong `facet_wrap()` bằng cách chỉ định "free_x" hoặc "free_y" hoặc "free".

**Số cột và hàng của biểu đồ con**\
Điều này có thể xác định với `ncol =` và `nrow =` trong `facet_wrap()`.
...
**Đinh dạng trục**\
Kích thước phông chữ và mặt biểu đồ, dải màu, v.v. có thể được sửa đổi thông qua `theme()` với các argument như:
...
**Dải nhãn**\
Nhãn của các biểu đồ con có thể được sửa đổi thông qua "nhãn" của cột như một hệ số hoặc bằng cách sử dụng của "labeller".

Tạo labeller như thế này, sử dụng hàm `as_labeller()` từ **ggplot2**. Sau đó, cung cấp labeller cho argument `labeller =` của `facet_wrap()` như hình dưới đây.
...
Để hiển thị tổng vụ dịch trong nền của mỗi biểu đồ con, hãy thêm hàm `gghighlight()` với dấu ngoặc đơn trống vào ggplot. Đây là từ package **gghighlight**. Lưu ý rằng trục y tối đa trong tất cả các biểu đồ con hiện dựa vào đỉnh của toàn bộ vụ dịch. Có nhiều ví dụ hơn về package này trong chương [Các tips với ggplot].
...
Dưới đây là tính năng trợ giúp. Chạy nó và luôn có sẵn.
...
Dữ liệu gần đây nhất được biểu thị trong đường cong dịch tễ nên được đánh dấu là tạm thời hoặc có thể báo cáo chậm trễ. Điều này có thể được thực hiện bằng cách thêm một đường thẳng đứng và/hoặc hình chữ nhật trong một số ngày cụ thể. Đây là hai tùy chọn:
...
[***LƯU Ý***: Bạn có thể thử `geom_rect()` để vẽ hình chữ nhật, nhưng việc điều chỉnh độ trong suốt không khả thi trong bối cảnh cảnh bộ số liệu `linelist`. Chức năng này sẽ phủ lên một hình chữ nhật cho mỗi hàng/quan sát!. Sử dụng hệ số alpha rất thấp (ví dụ: 0,01) hoặc một cách tiếp cận khác.]{style="color: orange;"}

### Sử dụng `annotate()` {.unnumbered}

-   Trong `annotate(geom = "rect")`, argument `xmin` và `xmax` cần được định dạng phân lớp ngày\
...
Đường thẳng đứng màu đen có thể tạo ra với code bên dưới, nhưng sử dụng `geom_vline()`, bạn sẽ mất khả năng kiểm soát đường cao:
...
Hãy nhớ rằng - bạn có thể sử dụng các công cụ như `\n` *trong* argument `date_labels` hoặc `labels` để đặt các phần của mỗi nhãn trên một dòng mới bên dưới. Tuy nhiên, đoạn code dưới đây giúp bạn thực hiện nhiều năm hoặc tháng (ví dụ) ở dòng thấp hơn *và chỉ một lần*. Một số lưu ý về code bên dưới:

-   Số lượng ca bệnh được tổng hợp thành các tuần vì lý do định dạng. Xem chương Đường cong dịch tễ (tab dữ liệu tổng hợp) để biết chi tiết.\
-   Một `geom_area()` dòng được sử dụng thay vì biểu đồ, vì phương pháp tiếp cận nhiều mặt dưới đây không hoạt động tốt với biểu đồ.
...
-   Các biểu đồ phải có cùng trục x chính xác (đã đặt giới hạn), nếu không dữ liệu và nhãn sẽ không được căn chỉnh phù hợp\
-   Sử dụng `theme_cowplot()` và một cái có trục y được di chuyển sang phía bên phải của biểu đồ
...
Lưu ý: Nếu sử dụng **incidence2**, hãy xem chương về cách bạn có thể tính tỷ lệ tích lũy bằng một hàm đơn giản. Chương này sẽ đề cập đến cách tính tỷ lệ tích lũy và vẽ biểu đồ bằng `ggplot()`.
