...
Chương này mô tả các cách "nối (join)", "đối sánh (match)", "liên kết (link), "ràng buộc (bind)" và kết hợp các data frame.
...
Trong chương này chúng tôi trình bày code nhằm:
...
-   Mở rộng một data frame bằng cách *liên kết* hoặc ("nối") trực tiếp các hàng hoặc cột từ một data frame khác

<!-- ======================================================= -->

## Chuẩn bị

### Tải package {.unnumbered}

Đoạn code này hiển thị những package cần tải cho các phân tích. Trong sổ tay này, chúng tôi nhấn mạnh đến `p_load()` từ **pacman**, hàm sẽ cài đặt package nếu cần *và* tải nó để sử dụng. Bạn cũng có thể tải các package đã cài đặt với `library()` từ **base** R. Xem chương [R cơ bản] để có thêm thông tin về các R package.
...
50 hàng đầu tiên của linelist được hiển thị dưới đây. The first 50 rows of the linelist are displayed below.
...
Trong phần nối dữ liệu dưới đây, chúng tôi sẽ sử dụng các bộ dữ liệu sau:

1)  Một phiên bản "thu nhỏ" của `linelist` trường hợp, chỉ chứa các cột `case_id`, `date_onset`, `hospital` và chỉ 10 hàng đầu tiên\
2)  Một data frame riêng biệt mà có tên `hosp_info`, chứa thêm thông tin chi tiết của từng bệnh viện

Trong phần đối sánh theo xác suất, chúng ta sẽ sử dụng hai bộ dữ liệu nhỏ khác nhau. Code để tạo các bộ dữ liệu này được đưa ra trong phần đối sánh đó.

#### Linelist trường hợp "thu nhỏ" {#joins_llmini .unnumbered}

Dưới đây là linelist trường hợp thu nhỏ, linelist mà chỉ chứa 10 hàng và chỉ chứa các cột `case_id`, `date_onset` và `hospital`.
...
Các phương pháp nối truyền thống (không theo xác suất) phân biệt chữ hoa, chữ thường và yêu cầu khớp các ký tự chính xác giữa các giá trị trong hai data frame. Để trình bày một số bước làm sạch mà bạn có thể cần làm trước khi bắt đầu nối, chúng tôi sẽ làm sạch và căn chỉnh bộ dữ liệu `linelist_mini` và `hosp_info` ngay bây giờ.

**Xác định điểm khác biệt**

Chúng tôi cần các giá trị của cột `hosp_name` trong data frame `hosp_info` để khớp với các giá trị của cột `hospital` trong data frame `linelist_mini`.
...
Chúng tôi bắt đầu bằng cách làm sạch các giá trị trong data frame `hosp_info`. Như đã được giải thích trong chương [Làm sạch số liệu và các hàm quan trọng], chúng ta có thể code lại các giá trị với tiêu chí logic bằng cách sử dụng hàm `case_when()` của **dplyr**. Đối với bốn bệnh viện tồn tại trong cả hai data frame, chúng tôi thay đổi các giá trị để phù hợp với các giá trị trong `linelist_mini`. Các bệnh viện khác chúng tôi để nguyên giá trị (`TRUE ~ hosp_name`).

[***CHÚ Ý:*** Thông thường khi làm sạch, người ta nên tạo một cột mới (ví dụ: `hosp_name_clean`), nhưng để dễ dàng giải thích, chúng tôi để hiển thị cột cũ đã được sửa đổi]{style="color: orange;"}
...
Tên bệnh viện xuất hiện trong cả hai data frame đều được căn chỉnh. Có hai bệnh viện trong `hosp_info` không có trong `linelist_mini` - chúng tôi sẽ giải quyết những trường hợp này sau, trong phần nối dữ liệu.

```{r}
unique(hosp_info$hosp_name)
```

Trước một phép nối, việc chuyển đổi một cột thành tất cả chữ thường hoặc tất cả chữ hoa thường dễ dàng nhất. Nếu bạn cần chuyển đổi tất cả các giá trị trong một cột thành CHỮ HOA hoặc chữ thường, hãy sử dụng `mutate()` và đặt cột bên trong một trong những hàm từ **stringr**, đã được trình bày trong chương [Ký tự và chuỗi].
...
Package **dplyr** cung cấp một số hàm nối khác nhau. **dplyr** nằm trong package **tidyverse**. Các hàm nối này được mô tả ở bên dưới, trong các trường hợp sử dụng đơn giản.

Rất cảm ơn [https://github.com/gadenbuie](https://github.com/gadenbuie/tidyexplain/tree/master/images) vì những gif bổ ích!

<!-- ======================================================= -->

### Cú pháp chung {.unnumbered}

Các lệnh nối có thể được chạy dưới dạng các lệnh độc lập để nối hai data frame thành một đối tượng mới, hoặc chúng có thể được sử dụng trong một chuỗi pipe (`%>%`) để hợp nhất một data frame vào một data frame khác khi nó đang được làm sạch hoặc chỉnh sửa theo hướng khác.
...
Nếu bạn đang nối các data frame dựa trên các giá trị chung của nhiều trường, hãy liệt kê các trường này trong vectơ `c()`. Ví dụ này nối các hàng nếu các giá trị trong ba cột trong mỗi bộ dữ liệu căn chỉnh chính xác.
...
Các lệnh nối cũng có thể được chạy trong một chuỗi pipe. Điều này sẽ sửa đổi data frame trở thành pipe hóa.

Trong ví dụ dưới đây, `df1` được đưa qua các pipe, `df2` được nối vào đó và vì thế `df` được chỉnh sửa và xác định lại.
...
[***CHÚ Ý:*** Nối là những trường hợp cụ thể! Do đó, rất hữu ích khi chuyển đổi tất cả các giá trị thành chữ thường hoặc chữ hoa trước khi nối. Xem chương về ký tự/chuỗi.]{style="color: orange;"}
...
Khi sử dụng các phép nối này, thứ tự bằng văn bản của các data frame trong lệnh rất quan trọng\*.
...
Các ví dụ động về phép nối trái và phải ([nguồn ảnh](https://github.com/gadenbuie/tidyexplain/tree/master/images))
...
Dưới đây là kết quả đầu ra của `left_join()` trong `hosp_info` (data frame thứ cấp, [xem tại đây](#joins_hosp_info)) *vào* `linelist_mini` (data frame cơ sở, [xem tại đây](#joins_llmini)). `linelist_mini` gốc có hàng `nrow(linelist_mini)`. `linelist_mini` đã chỉnh sửa được hiển thị. Lưu ý những điều dưới đây:
...
Dưới đây kết quả đầu ra việc nối `full_join()` của `hosp_info` (ban đầu là `nrow(hosp_info)`, [xem tại đây](#joins_hosp_info)) *vào* `linelist_mini` (ban đầu là `nrow(linelist_mini)`, [xem tại đây](#joins_llmini)). Lưu ý những điều dưới đây:
...
### Nối anti (Anti join) {.unnumbered}

**Nối anti là một "phép nối chọn lọc" khác trả về các hàng trong data frame cơ sở *không* khớp với data frame thứ cấp.**

Đọc thêm về những phép nối "chọn lọc" này [tại đây](https://towardsdatascience.com/level-up-with-semi-joins-in-r-a068426096e0).

Các tình huống phổ biến cho một phép nối-anti bao gồm xác định các bản ghi không tồn tại trong một data frame khác, khắc phục lỗi chính tả trong một phép nối (xem xét các bản ghi *đáng lẽ sẽ* khớp) và kiểm tra các bản ghi đã bị loại trừ sau một phép nối khác.
...
Ví dụ động về phép nối anti ([nguồn ảnh](https://github.com/gadenbuie/tidyexplain/tree/master/images))

#### Ví dụ `anti_join()` đơn giản {.unnumbered}

Một ví dụ đơn giản, hãy tìm bệnh viện trong `hosp_info` mà không tồn tại trong `linelist_mini`. Chúng tôi liệt kê `hosp_info` trước, như một data frame cơ sở. Các bệnh viện không có trong `linelist_mini` sẽ được trả về.
...
Một ví dụ khác, giả sử chúng tôi đã chạy một `inner_join()` giữa `linelist_mini` và `hosp_info`. Lệnh này chỉ trả về một tập con các bản ghi của `linelist_mini` gốc, vì một số bản ghi không có trong `hosp_info`.
...
Để xem lại các bản ghi `linelist_mini` đã bị loại trừ trong phép nối bên trong, chúng tôi có thể chạy một phép nối-anti với cùng thiết lập (`linelist_mini` là data frame cơ sở).
...
Để xem các bản ghi `hosp_info` đã bị loại trừ trong phép nối bên trong, chúng tôi cũng có thể chạy một phép nối-anti với `hosp_info` là data frame cơ sở.
...
Dưới đây là hai bộ dữ liệu mẫu nhỏ mà chúng tôi sẽ sử dụng để giải thích phương pháp khớp theo xác suất (`cases` và `test_results`):
...
**Bộ dữ liệu `test_results`** có 14 bản ghi và chứa cột `result`, cột mà chúng tôi muốn thêm vào các bản ghi trong `cases` dựa trên các bản ghi khớp theo xác xuất.
...
Chúng tôi đã định nghĩa đối tượng được trả về từ `fastLink()` là `fl_output`. Nó thuộc nhóm `list`, và nó thực sự chứa một số data frame bên trong nó, mô tả chi tiết kết quả của việc so khớp. Một trong những data frame này là `matches`, chứa các kết quả khớp có nhiều khả năng nhất giữa `cases` và `results`. Bạn có thể truy cập data frame "khớp" này với `fl_output$matches`. Dưới đây, nó được lưu dưới dạng `my_matches` để tiện cho việc truy cập sau này.
...
Ở đây chúng tôi đã bắt đầu với bộ dữ liệu `cases`, nhưng bây giờ đang được gọi là `cases_dup`, vì nó có 2 hàng bổ sung mà có thể là bản trùng lặp của các hàng trước đó: Xem "Tony" với "Anthony", và "Marialisa Rodrigues" với "Maria Rodriguez".
...
Chạy `fastLink()` giống nhu trước, nhưng so sánh data frame `cases_dup` với chính nó. Khai hai data frames được cung cấp giống hệt nhau, hàm sẽ giả định rằng bạn muốn loại bỏ trùng lặp. Lưu ý rằng chúng tôi không chỉ định `stringdist.match =` hoặc `numeric.match =` như chúng tôi đã làm trước đây.
...
## Ràng buộc và căn chỉnh

Một phương pháp khác để kết hợp hai data frame là "ràng buộc" chúng với nhau. Bạn cũng có thể coi đây là hàng hoặc cột được "nối vào" hoặc "thêm".

Phần này cũng sẽ thảo luận về cách "căn chỉnh" thứ tự các hàng của một data frame với thứ tự các hàng trong data frame khác. Chủ đề này được thảo luận dưới đây trong phần về Ràng buộc cột.

### Ràng buộc các hàng {.unnumbered}

Để liên kết các hàng của một data frame này với phần cuối của một data frame khác, hãy sử dụng `bind_rows()` từ **dplyr**. Nó rất bao hàm, vì vậy bất kỳ cột nào có trong một trong hai data frame sẽ được đưa vào kết quả đầu ra. Một số lưu ý:
...
Một ví dụ phổ biến về ràng buộc hàng là liên kết một hàng "tổng (total)" vào một bảng mô tả được tạo bằng hàm `summarise()` của **dplyr**. Dưới đây, chúng tôi tạo một bảng đếm số trường hợp và giá trị CT trung bình theo bệnh viện với một hàng tổng.
...
Bây giờ chúng ta có thể ràng buộc các hàng với nhau bằng `bind_rows()`.
...
Ví dụ, chúng tôi liên kết một số bảng tóm tắt với nhau. Để làm điều này, chúng tôi cũng trình bày cách sắp xếp lại thứ tự của các hàng trong một data frame để khớp với thứ tự hàng trong một data frame khác, với `match()`.

Ở đây chúng tôi định nghĩa `case_info` là một data frame tóm tắt về các trường hợp trong linelist theo bệnh viện, với số trường hợp và số ca tử vong.
...
Do thứ tự hàng khác nhau, một lệnh `bind_cols()` đơn giản sẽ dẫn đến khớp sai dữ liệu. Để khắc phục điều này, chúng ta có thể sử dụng `match()` từ **base** R để căn chỉnh các hàng của data frame theo thứ tự giống với thứ tự trong data frame khác. Chúng tôi giả định đối với phương pháp này rằng không có giá trị trùng lặp nào trong cả hai data frame.
...
Bây giờ chúng ta có thể ràng buộc các cột data frame với nhau, với thứ tự hàng chính xác. Lưu ý rằng một số cột bị trùng lặp và sẽ yêu cầu làm sạch bằng `rename()`. Đọc thêm về `bind_rows()` [tại đây](https://dplyr.tidyverse.org/reference/bind.html).
