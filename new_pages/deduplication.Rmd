...
3.  "Cuộn-lên" hoặc kết hợp các giá trị từ nhiều hàng thành một hàng
...
-   Một vài cá nhân có nhiều cuộc gọi truy vết, vào những ngày, giờ khác nhau, như những liên hệ và/hoặc như những trường hợp\
-   Ở mỗi cuộc gọi truy vết, người được truy vết được hỏi đã **từng** mắc các triệu chứng chưa và một số thông tin này bị missing.

Và đây là bản tóm tắt nhanh về những người truy vết và mục đích các cuộc gọi truy vết của họ, sử dụng `tabyl()` từ **janitor**:
...
Để chỉ giữ lại các hàng duy nhất của một data frame, hãy sử dụng `distinct()` từ **dplyr** (đã được giải thích trong chương [Làm sạch số liệu và các hàm quan trọng]). Các hàng trùng lặp sẽ bị loại bỏ sao cho chỉ giữ lại hàng đầu tiên trong số các hàng đó. Theo mặc định, hàng "đầu tiên" có nghĩa là hàng có `rownumber` cao nhất (thứ tự của các hàng từ trên-xuống-dưới). Chỉ còn lại các hàng duy nhất.

Trong ví dụ dưới đây, chúng tôi chạy lệnh `distinct()` sao cho cột `recordID` bị loại trừ khỏi việc xem xét - do đó **hai hàng trùng lặp sẽ bị loại bỏ**. Hàng đầu tiên (đối với "adam") bị trùng lặp 100% và đã bị loại bỏ. Bên cạnh đó, hàng 3 (cho "amrish") là một bản bị trùng lặp trong tất cả các cột *ngoại trừ* `recordID` (đang không được xét) và do đó cũng bị loại bỏ. Bộ dữ liệu `obs` bây giờ có dòng `nrow(obs)-2`, không phải `nrow(obs)`).
...
Để chỉ trả về các phần tử duy nhất, hãy sử dụng `unique()` từ **base** R. Để loại bỏ các `NA` khỏi kết quả đầu ra, hãy lồng `na.omit()` trong `unique()`.
...
Để xác định các cột, hãy cung cấp số hoặc tên cột sau dấu phẩy (nhớ rằng đặt tất cả những thông tin này *trong* hàm `duplicated()`).
...
Để "cắt" một data frame nhằm áp dụng bộ lọc trên các hàng theo số/vị trí hàng. Điều này trở nên đặc biệt hữu ích nếu bạn có nhiều hàng cho mỗi nhóm chức năng (ví dụ: mỗi "người") và bạn chỉ muốn giữ một hoặc một số hàng trong số đó.
...
Có một số biến thể: Chúng phải được cung cấp với một cột và một số hàng để trả về (thành `n =`).

-   `slice_min()` và `slice_max()` chỉ giữ (các) hàng có (các) giá trị nhỏ nhất hoặc lớn nhất của cột được chỉ định. Hàm này cũng hoạt động để trả về "tối thiểu" và "tối đa" của các yếu tố có thứ tự.\
...
`.order_by =` được sử dụng trong `slice_min()` và `slice_max()`, đây là một cột nhằm sắp xếp thứ tự trước khi cắt.\
...
`weight_by =` Tùy chọn, cột số theo trọng số (số lớn hơn có nhiều khả năng được lấy mẫu hơn). Đồng thời `replace =` cho việc lấy mẫu liệu có được thực hiện mà có/không có sự thay thế.

[***MẸO:*** Khi sử dụng `slice_max()` và `slice_min()`, hãy chắc chắn xác định/ghi `n =` (ví dụ: `n = 2`, không chỉ `2`). Nếu không, bạn có thể gặp lỗi `Error:`...`is not empty`.]{style="color: darkgreen;"}
...
Trong ví dụ dưới đây, để chỉ giữ lại cuộc gọi truy vết *mới nhất* *theo mỗi người*, chúng tôi nhóm các hàng theo cột `name` và sau đó sử dụng `slice_max()` với `n = 1` trên cột `date`. Hãy lưu ý! Để áp dụng một hàm như `slice_max()` vào dates (ngày tháng), cột ngày tháng phải thuộc nhóm Date.
...
[***THẬN TRỌNG:*** Nếu `with_ties = FALSE`, hàng đầu tiên có ràng buộc được giữ lại. Điều này có thể là sự nhầm lẫn. Hãy xem trường hợp của Mariah, cô ấy có hai cuộc gọi truy vết vào ngày gần nhất (ngày 06/01) và cuộc gọi truy vết đầu tiên (sớm nhất) được giữ lại. Rất có thể, chúng tôi muốn giữ lại cuộc gọi truy vết muộn hơn của cô ấy vào ngày đó. Xem cách "phá vỡ" những ràng buộc này trong ví dụ tiếp theo.]{style="color: red;"}
...
Sau đó, cột mới `key_completeness` được tạo với `mutate()`. Giá trị mới của mỗi hàng được xác định dưới dạng phân số được tính toán bằng: số giá trị không bị thiếu trong hàng đó trong số các cột chính, chia cho số cột chính.

Điều này liên quan đến hàm `rowSums()` từ **base** R. Cũng được sử dụng là `.`, trong tham chiếu pipe đến data frame tại một điểm trong pipe (trong trường hợp này, nó đang là tập con với dấu ngoặc `[]`).
...
## Giá trị cuộn lên {#str_rollup}

Phần này miêu tả:

1)  Cách "cuộn" các giá trị từ nhiều hàng chỉ thành một hàng, với một số biến thể\
2)  Khi bạn có các giá trị "đã được cuộn lên", cách để ghi đè/ưu tiên các giá trị trong mỗi ô

Tab này sử dụng bộ dữ liệu mẫu từ tab Chuẩn bị.

<!-- ======================================================= -->

### Cuộn các giá trị thành một hàng {.unnumbered}
...
Trong trường hợp này, "\_roll" được dùng để biểu thị rằng nó đã được cuộn:
...
Sau đó, nếu bạn muốn đánh giá tất cả các giá trị đã cuộn và chỉ giữ một giá trị cụ thể (ví dụ: giá trị "tốt nhất (best)" hoặc "tối đa (maximum)"), bạn có thể sử dụng `mutate()` trên các cột mong muốn, để triển khai `case_when()`, hàm sử dụng `str_detect()` từ package **stringr** để tìm kiếm tuần tự các mẫu chuỗi và ghi đè nội dung ô.
...
Xem chương [Nối dữ liệu] để được giải thích về phương pháp này. Mục Đối sánh theo xác suất chứa một ví dụ về việc áp dụng các thuật toán này để so sánh một data frame với *chính nó*, do đó thực hiện loại bỏ trùng lặp theo xác suất.
