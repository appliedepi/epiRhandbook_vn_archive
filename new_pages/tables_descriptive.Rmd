...
### Tổng hàng {#tbl_dplyr_totals .unnumbered}

When `summarise()` operates on grouped data it does not automatically produce "total" statistics. Dưới đây, hai cách tiếp cận để thêm tổng hàng được trình bày:
...
Test pull request: To get the totals, run the same `summarise()` command but only group the data by outcome (not by hospital), like this:
