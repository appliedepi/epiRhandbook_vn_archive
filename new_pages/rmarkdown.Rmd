...
Một bản thảo R Markdown có rải rác các code R và chữ khiến cho các bản thảo thật sự *trở thành một tài liệu đầu ra*. Bạn có thể tạo ra một tài liệu đã được định dạng toàn bộ, gồm có chữ thuần túy (có thể là động lực để thay đổi dựa trên số liệu của bạn), bảng, biểu đồ, đầu mục/số, thư mục, vân vân.   
...
Những chương khác trong quyển sổ tay này mở rộng các chủ đề khác:  

* Trang [Sắp xếp các báo cáo định kì] trình bày cách tự động hóa các báo cáo sản phẩm với các tập tin tự tạo kèm theo thời gian.  
* Trang [Dashboards với R Markdown] giải thích cách định dạng một báo cáo R Markdown dưới dạng bảng điều khiển.  


Một lưu ý là dự án [R4Epis](https://r4epis.netlify.app/) đã tạo ra các mẫu bản thảo R Markdown cho những đợt dịch và trường hợp khảo sát được đặt ở vị trí dự án MSF.  
...
Nhằm giải thích một số khái niệm và packages gồm có:
...
* **R Markdown**: là một biến thể trên markdown _chỉ dành riêng cho R_ - nó cho phép bạn soạn một tài liệu dùng markdown để tạo chữ, *nhúng code R và hiển thị kết quả đầu ra*. Các tệp R Markdown có đuôi '.Rmd'.   
...
* **knitr**: Package R này sẽ đọc đoạn code, thực thi chúng, và 'knit' (kết hợp) chúng vào lại tài liệu. Đây là cách bảng, biểu đồ được đưa vào cùng chữ.  
...
Tổng kết lại, quá trình được tiến hành *trong nền* (bạn không cần biết tới tất cả những bước này!), gồm chuyển tệp .Rmd thành **knitr** để thực thi các đoạn code R và tạo một tệp .md (markdown) mới có cả code R lẫn kết quả đầu ra đã được chuyển. Các tệp .md này sau đó được pandoc chạy để tạo ra sản phẩm hoàn thiện: Một tài liệu Microsoft Word, tệp HTML, tài liệu powerpoint, pdf, v.v.  
...
Để tạo một kết quả đầu ra R Markdown, bạn cần phải cài đặt:  
...
* Nếu bạn muốn tạo đầu ra PDF (phức tạp hơn một chút), bạn sẽ cần cài đặt LaTex. Với những người dùng R Markdown chưa cài đặt LaTex trước đó, các bạn có thể cài đặt TinyTeX (https://yihui.name/tinytex/). Bạn có thể sử dụng đoạn lệnh sau:

```{r, eval=F}
pacman::p_load(tinytex)     # cài đặt tinytex package
tinytex::install_tinytex()  # Lệnh R để cài đặt phần mềm TinyTeX 
```

<!-- ======================================================= -->
## Bắt đầu {  }

### Cài đặt R package rmarkdown  {.unnumbered}

Cài đặt R package **rmarkdown** . Trong quyển sổ tay này chúng tôi nhấn mạnh `p_load()` từ **pacman**, cài đặt package nếu cần *và* gọi sử dụng. Bạn cũng có thể cài đặt package với `library()` từ **base** R. Xem thêm ở chương [R cơ bản] để biết thêm thông tin về R packages.  
...
R Studio sẽ đưa ra cho bạn một số lựa chọn kết quả đầu ra. Trong ví dụ bên dưới, chúng tôi chọn "HTML" bởi chúng tôi muốn tạo ra một văn bản html. Tiêu đề và tên các tác giả không quan trọng. Nếu loại tài liệu đầu ra bạn muốn không có, đừng lo - bạn có thể chọn bất kỳ loại nào và thay đổi trong bản thảo sau đó.  
...
Thư mục làm việc của một tệp markdown là ở bất kỳ vị trí nào tệp được lưu. Ví dụ, nếu R project nằm trong `~/Documents/projectX ` và tệp Rmd được lưu ở thư mục con `~/Documents/projectX/markdownfiles/markdown.Rmd`, code `read.csv(“data.csv”)` trong markdown sẽ tìm tệp csv trong thư mục `markdownfiles`, mà không phải là ở thư mục nguồn nơi scripts trong projects sẽ thường tự động xem.  

Để nhắc tới các tệp này ở chỗ khác, bạn sẽ vừa cần sử dụng cả đường dẫn tệp đầy đủ và sử dụng package **here**. Package **here** cài đặt thư mục làm việc tới thư mục nguồn của R project và sẽ được giải thích chi tiết trong các chương [Dự án R] và [Nhập xuất dữ liệu] của sổ tay này. Ví dụ, để nhập một tệp tên "data.csv" từ trong thư mục `projectX`, code sẽ là `import(here(“data.csv”))`.  

Lưu ý rằng không nên sử dụng `setwd()` trong R Markdown scripts - nó chỉ dùng cho các đoạn code được viết trong đó. 

**Làm việc với ổ cứng so với trên máy tính của bạn**

Vì R Markdown có thể gặp phải vấn đề pandoc khi chạy trên một ổ cứng mạng được chia sẻ, thư mục của bạn nên nằm ở máy gốc, ví dụ trong dự án thuộc 'My Documents'. Nếu bạn dùng Git (rất khuyến khích!), nó sẽ thân thuộc hơn. Để biết thêm chi tiết, xem thêm chương [R trên ổ cứng mạng] và [Các lỗi và sự trợ giúp].  


<!-- ======================================================= -->
## Các cấu phần của R Markdown {  }

Một tài liệu R Markdown có thể sửa trong RStudio giống như một R script tiêu chuẩn. Khi bạn tạo một R Markdown script mới, RStudio tỏ ra hữu ích bằng cách đưa ra một mẫu có các phần khác nhau của một R Markdown script.  
...
YAML nên bắt đầu với metadata cho tài liệu. Thứ tự của những tham số YAML chính (không thụt dòng) không quan trọng. Ví dụ:  
...
Đây là phần tường thuật tài liệu của bạn, gồm các tiêu đề và đề mục. Nó được viết bằng ngôn ngữ "markdown", được sử dụng trên nhiều phần mềm khác nhau.  

Dưới đây là những cách cốt lõi để viết văn bản này. Xem thêm tài liệu mở rộng có sẵn trên R Markdown "cheatsheet" tại [Trang web RStudio](https://rstudio.com/resources/cheatsheets/).  
...
Không có cơ chế đơn giản nào để thay đổi màu chữ trong R Markdown. Một giải pháp khác, *NẾU đầu ra của bạn là tệp HTML*, là thêm một dòng HTML vào văn bản markdown. Đoạn mã HTML dưới đây sẽ in một dòng văn bản có màu đỏ đậm.  
...
Dấu băm trong phần văn bản của một script R Markdown tạo ra đầu mục. Điều này khác với một đoạn code R trong script, trong đó ký hiệu băm là cơ chế nhận xét/chú thích/hủy kích hoạt, như trong R script bình thường.  

Các mức tiêu đề khác nhau được thiết lập với số lượng ký hiệu băm khác nhau khi bắt đầu một dòng mới. Một ký hiệu băm là tiêu đề hoặc đầu mục chính. Hai ký hiệu băm là một đầu mục cấp hai. Các đầu mục cấp ba và cấp bốn có thể được tạo bằng các ký hiệu băm liên tiếp.  
...
Bạn có thể "chú giải" văn bản R Markdown giống như bạn sử dụng "#" để chú giải một dòng code R trong một đoạn R. Chỉ cần đánh dấu văn bản và nhấn Ctrl+Shift+c (Cmd+Shift+c cho Mac). Văn bản sẽ được bao quanh bởi các mũi tên và chuyển sang màu xanh lục. Nó sẽ không xuất hiện trong đầu ra của bạn.  
...
Các phần của tập lệnh dành riêng để chạy code R được gọi là "đoạn". Đây là nơi bạn có thể tải các package, nhập dữ liệu và thực hiện quản lý và trực quan hóa dữ liệu thực tế. Có thể có nhiều đoạn code, vì vậy chúng có thể giúp bạn tổ chức code R của mình thành các phần, có thể xen kẽ với văn bản. Cần lưu ý:  
Những 'đoạn' này sẽ có màu nền hơi khác so với phần tường thuật của tài liệu.  
...
Bạn có thể tạo một đoạn code mới bằng cách tự gõ nó ra, bằng cách sử dụng phím tắt "Ctrl + Alt + i" (hoặc Cmd + Shift + r trong Mac), hoặc bằng cách nhấp vào biểu tượng 'chèn đoạn code mới' màu xanh lục ở đầu trình chỉnh sửa tập lệnh của bạn.  

Một số lưu ý về nội dung của dấu ngoặc nhọn `{ }`:

*	Chúng bắt đầu bằng ‘r’ để chỉ ra rằng tên ngôn ngữ trong đoạn này là R
*	Sau chữ r, bạn có thể tùy ý viết "tên" đoạn - những thứ này không cần thiết nhưng có thể giúp bạn sắp xếp công việc của mình. Lưu ý rằng nếu bạn đặt tên cho các phần của mình, bạn phải LUÔN sử dụng các tên độc nhất, nếu không R sẽ báo lại khi bạn cố gắng kết xuất.  
*	Dấu ngoặc nhọn cũng có thể bao gồm các tùy chọn khác, được viết dưới dạng `tag = value`, chẳng hạn như:  
  ...
  * `message = FALSE` để không hiển thị bất kỳ tin nhắn nào được tạo bởi code R  
  * `include =` TRUE/FALSE có bao gồm đầu ra đoạn code (ví dụ: các đồ thị) trong tài liệu hay không  
  * `out.width = ` và `out.height =` - đưa ra theo dạng `out.width = "75%"`  
  * `fig.align = "center"` điều chỉnh cách một hình được căn trên trang  
  * `fig.show='hold'` nếu đoạn code của bạn hiển thị nhiều biểu đồ và bạn muốn chúng hiển thị cạnh nhau  (cùng với `out.width = c("33%", "67%")`. Có thể đặt là `fig.show='asis'` để hiển thị chúng dưới code tạo chúng, `'hide'` để ẩn, hoặc `'animate'` để nối nhiều cái thành một hoạt hình.  
* Tiêu đề đoạn phải được viết trong *một dòng*   
...
#### Code R trong đoạn chữ {.unnumbered}  
...
### Các phần được gắn tab {.unnumbered}  


Đối với đầu ra HTML, bạn có thể sắp xếp các phần thành "tab". Chỉ cần thêm `.tabset` vào trong dấu ngoặc nhọn` {} `được đặt *sau đề mục*. Bất kỳ đề mục phụ nào bên dưới tiêu đề đó (cho đến khi tiêu đề khác cùng cấp) sẽ xuất hiện dưới dạng các tab mà người dùng có thể nhấp qua. Đọc thêm [tại đây](https://bookdown.org/yihui/rmarkdown-cookbook/html-tabs.html)  
...
Bạn có thể thêm một tùy chọn bổ sung `.tabset-Drugs` sau` .tabset` để tạo cho các tab có giao diện "xếp chồng". Lưu ý rằng khi xem đầu ra HTML theo tab, chức năng tìm kiếm Ctrl+f sẽ chỉ tìm kiếm các tab "đang hoạt động" chứ không phải các tab ẩn.  
...
* R Markdown khép kín - mọi thứ cần thiết cho báo cáo đều được nhập hoặc tạo trong R Markdown  
  * Nguồn các tệp khác - Bạn có thể chạy các script R bên ngoài bằng lệnh `source ()` và sử dụng đầu ra của chúng trong Rmd  
  * Scipt con - một cơ chế thay thế cho `source()`  
* Sử dụng "runfile" - Chạy các lệnh trong script R *trước khi* kết xuất R Markdown  
...
Mọi thứ bạn cần để chạy R markdown đều được nhập hoặc tạo trong tệp Rmd, bao gồm tất cả các đoạn code và package. Cách tiếp cận "khép kín" này phù hợp khi bạn không cần xử lý nhiều dữ liệu (ví dụ: nó mang đến một tệp dữ liệu sạch hoặc bán sạch) và việc kết xuất R Markdown sẽ không mất quá nhiều thời gian.  
...
Một giải pháp thay thế là sử dụng tùy chọn `child = ` **knitr**. GIẢI THÍCH THÊM VIỆC LÀM
...
Ví dụ, bạn có thể tải các package, tải và làm sạch dữ liệu, và thậm chí tạo các biểu đồ trước khi `render()`. Các bước này có thể xảy ra trong R script hoặc trong các script khác được lưu nguồn. Miễn là các lệnh này xảy ra trong cùng một phiên RStudio và các đối tượng được lưu vào môi trường, các đối tượng sau đó có thể được gọi trong nội dung Rmd. Sau đó, bản thân R markdown sẽ chỉ được sử dụng cho bước cuối cùng - để tạo ra kết quả với tất cả các đối tượng được xử lý trước. Điều này dễ dàng hơn để gỡ lỗi nếu có sự cố.  
...
Quy trình làm việc cũng liên quan đến cấu trúc thư mục tổng thể, chẳng hạn như có thư mục 'đầu ra' cho các tài liệu và số liệu đã tạo và thư mục 'dữ liệu' hoặc 'đầu vào' cho dữ liệu đã được làm sạch. Chúng tôi không đi sâu vào chi tiết ở đây, nhưng hãy xem chương [Tổ chức báo cáo thường kỳ].  
...
Như file kết xuất, RStudio Console sẽ cho bạn thấy quá trình kết xuất tới 100%, và một thông báo cuối để báo rằng quá trình kết xuất đã hoàn thành.  


###  Cách 3: **reportfactory**  package {.unnumbered}  


R package **reportfactory** cung cấp một phương pháp thay thế để tổ chức và soạn báo cáo R Markdown *phù hợp với các tình huống mà bạn chạy báo cáo thường xuyên (ví dụ hàng ngày, hàng tuần ...).* Nó giúp giảm bớt việc soạn nhiều tệp R Markdown và tổ chức đầu ra của chúng. Về bản chất, nó cung cấp một "nhà máy" mà từ đó bạn có thể chạy báo cáo R Markdown, nhận các thư mục được đánh dấu ngày tháng và thời gian tự động cho kết quả đầu ra và có quyền kiểm soát phiên bản "nhẹ".  

Đọc thêm về quy trình công việc này trong chương về [Tổ chức các báo cáo thường kỳ].  
...
Bạn có thể sử dụng tham số hóa để tạo báo cáo động, sao cho báo cáo có thể được chạy với cài đặt cụ thể (ví dụ: ngày hoặc địa điểm cụ thể hoặc với các tùy chọn knit nhất định). Dưới đây, chúng tôi tập trung vào những điều cơ bản, nhưng có thêm [chi tiết trực tuyến](https://bookdown.org/yihui/rmarkdown/parameterized-reports.html) về các báo cáo được tham số hóa.  

Sử dụng danh sách dòng Ebola làm ví dụ, giả sử chúng ta muốn chạy một báo cáo giám sát tiêu chuẩn cho từng bệnh viện mỗi ngày. Chúng ta chỉ ra cách người ta có thể làm điều này bằng cách sử dụng các tham số.  
...
Chỉnh sửa YAML để bao gồm tùy chọn `params:`, với các câu lệnh thụt lề cho mỗi tham số bạn muốn xác định. Trong ví dụ này, chúng tôi tạo các tham số `date` và` hospital`, với các giá trị mà chúng tôi chỉ định. Các giá trị này có thể thay đổi mỗi khi chạy báo cáo. Nếu bạn sử dụng nút "Knit" để tạo ra đầu ra, các tham số sẽ có các giá trị mặc định này. Tương tự như vậy, nếu bạn sử dụng `render ()`, các tham số sẽ có các giá trị mặc định này trừ khi được chỉ định khác trong lệnh `render ()`.  
...
Lưu ý rằng các tham số cũng có thể giữ các giá trị `true` hoặc `false`, và vì vậy chúng có thể được đưa vào các tùy chọn **precisiontr** của bạn cho một đoạn R. Ví dụ: bạn có thể đặt `{r, eval=params$run}` thay vì `{r, eval=FALSE}`, và bây giờ việc đoạn chạy hay không phụ thuộc vào giá trị của tham số `run:`.  
...
Lệnh dưới đây hiển thị "tracking_report.Rmd", chỉ định tên và thư mục tệp đầu ra động, đồng thời cung cấp `list()` gồm hai tham số và giá trị của chúng cho đối số `params = `.  
...
#### Cách 3:  Đặt tham số dùng Giao diện Người dùng Đồ họa (Graphical User Interface) {.unnumbered}  

Để có cảm giác tương tác hơn, bạn cũng có thể sử dụng Giao diện Người dùng Đồ họa (Graphical User Interface - GUI) để chọn thủ công các giá trị cho các tham số. Để thực hiện việc này, chúng ta có thể nhấp vào menu thả xuống bên cạnh nút ‘Knit’ và chọn ‘Knit with parameter’.  
...
Dưới đây, các thông số kỹ thuật này được viết cho tham số `bệnh viện`.  
...
Trong kết quả đầu ra của báo cáo, hãy xem cách dữ liệu được lọc cho bệnh viện cụ thể và tiêu đề biểu đồ đề cập đến bệnh viện và ngày chính xác. Chúng tôi sử dụng tệp "linelist_cleaned.rds" ở đây, nhưng nó sẽ đặc biệt thích hợp nếu bản thân danh sách dòng cũng có một dấu dữ liệu bên trong nó để căn chỉnh với ngày được tham số hóa.  
...
Chúng ta có thể muốn chạy một báo cáo nhiều lần, thay đổi các thông số đầu vào, để tạo ra một báo cáo cho từng khu vực pháp lý/đơn vị. Điều này có thể được thực hiện bằng cách sử dụng các công cụ cho *lặp lại*, được giải thích chi tiết trong chương về [Lặp lại, vòng lặp và danh sách]. Các tùy chọn bao gồm package **purrr** hoặc sử dụng *vòng lặp for* như được giải thích bên dưới.  

Dưới đây, chúng tôi sử dụng  *vòng lặp for* đơn giản để tạo báo cáo giám sát cho tất cả các bệnh viện được chọn. Điều này được thực hiện bằng một lệnh (thay vì thay đổi từng thông số bệnh viện theo cách thủ công). Lệnh kết xuất báo cáo phải tồn tại trong một script riêng biệt *bên ngoài* báo cáo Rmd. Script này cũng sẽ chứa các đối tượng được xác định để "lặp qua" - ngày hôm nay và vectơ tên bệnh viện để lặp qua.
...
2) Với `params =` sao cho Rmd sử dụng tên bệnh viện trong nội bộ bất cứ khi nào giá trị `params$hospital` được gọi (ví dụ: chỉ để lọc tập dữ liệu cho bệnh viện cụ thể). Trong ví dụ này, bốn tệp sẽ được tạo - một tệp cho mỗi bệnh viện.  
...
Bằng cách sử dụng tài liệu mẫu có chứa bất kỳ định dạng mong muốn nào, bạn có thể điều chỉnh tính thẩm mỹ của đầu ra Rmd sẽ trông như thế nào. Ví dụ, bạn có thể tạo tệp MS Word hoặc Powerpoint chứa các trang/trang trình bày với kích thước, hình mờ, hình nền và phông chữ mong muốn.  
...
* visNetwork (được sử dụng trong chương [Chuỗi truyền] của sổ tay này)  
...
Một giải thích tốt về markdown so với knitr và Rmarkdown ở đây: https://stackoverflow.com/questions/40563479/relationship-between-r-markdown-knitr-pandoc-and-bookdown
