...
* Tạo các đường cong dịch bệnh cho nhiều phạm vi quyền hạn  
* Chạy T-tests cho nhiều cột trong dataframe  

Trong [phần](#iter_purrr) **purrr**, chúng tôi cũng sẽ cung cấp một số ví dụ về cách tạo và xử lý `lists`.    
...
Đoạn code này hiển thị cách tải các packages cần thiết cho phần phân tích. Trong sổ tay này, chúng tôi nhấn mạnh `p_load()` từ **pacman**, cài đặt package nếu cần *và* tải nó để sử dụng. Bạn cũng có thể tải các package đã cài đặt với `library()` từ **base** R. Xem chương trên [R cơ bản] để biết thêm thông tin về packages R.    
...
Cú pháp cơ bản là: `for (phần tử trong chuỗi) {các thao tác thực hiện với phần tử}`. Lưu ý dấu ngoặc đơn và dấu ngoặc nhọn. Kết quả có thể được in ra console hoặc được lưu trữ trong một đối tượng vùng chứa R.  
...
Đây là phần "for" của *vòng lặp for* - các thao tác sẽ chạy "for" qua từng phần tử trong chuỗi. Chuỗi có thể là một loạt các giá trị (ví dụ: tên của khu vực pháp lý, bệnh, tên cột, phần tử danh sách, v.v.) hoặc nó có thể là một chuỗi các số liên tiếp (ví dụ: 1,2,3,4,5). Mỗi cách tiếp cận được mô tả dưới đây có các tiện ích riêng của chúng.  

Cấu trúc cơ bản của câu lệnh chuỗi là `item in vector`.  
