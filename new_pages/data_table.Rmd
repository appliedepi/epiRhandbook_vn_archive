...
Cuốn sổ tay này tập trung vào các hàm "verb" **dplyr** và toán tử pipe **magrittr** `%>%` như một cách để làm sạch và nhóm dữ liệu, tuy nhiên package **data.table** cũng cung cấp một giải pháp thay thế mà bạn có thể sử dụng trong quá trình làm việc với R của mình.
...
Cấu trúc là **DT[i, j, by]**, được chia thành 3 phần; đối số **i, j** và **by**. Đối số **i** cho phép tạo số hàng cần thiết, đối số **j** cho phép bạn tạo số cột và đối số **by** cho phép bạn tạo số cột theo nhóm.
...
## Tải package và nhập dữ liệu

### Tải package {.unnumbered}

Sử dụng hàm `p_load()` từ **pacman**, chúng tôi tải (và cài đặt nếu cần) các package cần thiết cho phân tích này.
...
Chương này sẽ khám phá một số chức năng cốt lõi của **data.table** bằng cách sử dụng linelist trường hợp được tham chiếu trong sổ tay.

Chúng tôi nhập bộ dữ liệu về các trường hợp từ một vụ dịch Ebola mô phỏng. Nếu bạn muốn tải dữ liệu để làm theo từng bước, hãy xem hướng dẫn trong chương [Tải sách và dữ liệu]. Bộ dữ liệu được nhập bằng hàm `import()` từ package **rio**. Xem chương về [Nhập xuất dữ liệu] để biết các cách nhập dữ liệu khác nhau. Từ đây chúng tôi sử dụng `data.table()` để chuyển data frame thành data table.

```{r}
linelist <- rio::import(here("data", "linelist_cleaned.xlsx")) %>% data.table()
```

Hàm `fread()` được sử dụng để nhập trực tiếp các tệp được phân tách thông thường, chẳng hạn như tệp .csv, trực tiếp sang định dạng data table. Hàm này và đối tác của nó, `fwrite()`, được sử dụng để ghi data.table dưới dạng các tệp được phân tách thông thường là các tùy chọn rất nhanh và hiệu quả về mặt tính toán đối với cơ sở dữ liệu lớn.

20 hàng đầu tiên `linelist`:
...
Nhắc lại cấu trúc **DT[i, j, by]**, chúng tôi có thể lọc các hàng bằng cách sử dụng số hàng hoặc biểu thức logic. Đối số i là đối số đầu tiên; do đó, cú pháp **DT[i]** hoặc **DT[i,]** có thể được sử dụng.
...
Sử dụng .N trong đối số i đại diện cho tổng số hàng trong data table. Điều này có thể được sử dụng để thiết lập số hàng:
...
Data table sử dụng các hàm trợ giúp để việc thiết lập các hàng trở nên dễ dàng. Hàm `%like%` được sử dụng để khớp với một pattern trong cột, `%chin%` được sử dụng để khớp với một ký tự cụ thể và hàm trợ giúp `%between%` được sử dụng để khớp các cột số trong một phạm vi xác định trước.

Trong các ví dụ dưới đây, chúng tôi: \* lọc các hàng mà biến bệnh viện (hospital variable) có chứa "Hospital" \* lọc các hàng có kết quả là "Recover" hoặc "Death" \* lọc các hàng trong độ tuổi 40-60
...
Sử dụng cấu trúc **DT[i, j, by]**, chúng tôi có thể chọn cột bằng cách sử dụng số hoặc tên. Đối số **j** là đối số thứ hai; do đó, cú pháp **DT[, j]** được sử dụng. Để tạo điều kiện tính toán trên đối số **j**, cột được bao quanh bằng cách sử dụng `list()` hoặc `.()`.

### Chọn cột {.unnumbered}

Ví dụ đầu tiên lấy các cột thứ nhất, thứ ba và thứ năm của data table, ví dụ thứ hai chọn tất cả các cột ngoại trừ các cột chiều cao (height), cân nặng (weight) và giới tính (gender). Ví dụ thứ ba sử dụng `.()` bao quanh để chọn cột **case_id** và **outcome**.
...
Trong các ví dụ dưới đây, chúng tôi: \* Đếm số trường hợp nằm viện trên 7 ngày \* Tính tuổi trung bình của các trường hợp tử vong tại bệnh viện quân y (Military) \* Tính toán độ lệch chuẩn, trung vị, tuổi trung bình của các trường hợp đã khỏi bệnh tại Central Hospital
...
Trong các ví dụ dưới đây, chúng tôi:\
...
Trong các ví dụ này, chúng tôi đang tuân theo giả định rằng một hàng trong data table tương đương với một trường hợp mới, vì vậy chúng tôi có thể sử dụng **.N** để thể hiện số hàng trong data table. Một hàm hữu ích khác thể hiện số lượng các trường hợp duy nhất là `uniqueN()`, hàm này trả về số lượng các giá trị duy nhất trong một đầu vào nhất định.
