...
Hầu hết các packages R đều dựa trên [**survey** package](https://cran.r-project.org/web/packages/survey/index.html) cho phân tích có trọng số. Chúng tôi sẽ sử dụng **survey** cũng như [**srvyr**](https://cran.r-project.org/web/packages/srvyr/index.html) (một trình gọn hơn **survey** cho phép mã hóa theo kiểu tidyverse) và [**gtsummary**](https://cran.r-project.org/web/packages/gtsummary/index.html) (một trình bao bọc cho **survey** cho phép sẵn sàng xuất các bảng). Trong khi package **survey** ban đầu không cho phép mã hóa theo kiểu tidyverse, nó có thêm lợi ích là cho phép khảo sát được trọng số hóa mô hình tuyến tính tổng quát (sẽ được thêm vào chương này sau). Chúng tôi cũng sẽ chứng minh bằng cách sử dụng một hàm từ package [**sitrep**](https://github.com/R4EPI/sitrep) để tạo mẫu trọng số (*lưu ý* package này hiện chưa có trên CRAN, nhưng có thể được cài đặt từ github).

Hầu hết trang này dựa trên công việc hoàn thành cho [dự án "R4Epis"](https://r4epis.netlify.app/); để biết code chi tiết và mẫu R-markdown xem [github chương "R4Epis"](https://github.com/R4EPI/sitrep). Một số package **survey** dựa trên các phiên bản đầu tiên của [các nghiên cứuu trường hợp EPIET](https://github.com/EPIET/RapidAssessmentSurveys).

Hiện tại, chương này không đề cập đến các phép tính cỡ mẫu hoặc lấy mẫu. Để tính cỡ mẫu đơn giản, hãy xem [OpenEpi](https://www.openepi.com/Menu/OE_Menu.htm). Chương [GIS cơ bản](https://epirhandbook.com/gis-basics.html) của sổ tay sẽ có một phần về lấy mẫu ngẫu nhiên theo không gian và chương này cuối cùng sẽ có một phần về khung lấy mẫu cũng như kích thước mẫu các phép tính.
...
Đoạn code này hiển thị tải các packages cần thiết cho các phân tích. Trong sổ tay này, chúng tôi nhấn mạnh đến `p_load()` từ **pacman**, cài đặt packages nếu cần *và* tải nó để sử dụng. Bạn cũng có thể tải các packages với `library()` từ **base** R. Xem chương về [R cơ bản] để biết thêm thông tin về các packages R. Ở đây chúng tôi cũng chứng minh việc sử dụng hàm `p_load_gh()` từ **pacman** để cài đặt một package tải từ github chưa được công bố trên CRAN.
...
### Tải dữ liệu {.unnumbered}

Bộ dữ liệu ví dụ được sử dụng trong phần này:

-   dữ liệu điều tra tỷ lệ tử vong hư cấu.
-   số lượng dân số hư cấu cho khu vực khảo sát.
-   từ điển dữ liệu cho dữ liệu khảo sát tử vong hư cấu.

Bộ này dựa trên khảo sát đã được phê duyệt trước của hội đồng đạo đức MSF OCA. Các tập dữ liệu hư cấu được tạo như một phần của [Dự án "R4Epis"](https://r4epis.netlify.app/). Tất cả đều dựa trên dữ liệu được thu thập bằng [KoboToolbox](https://www.kobotoolbox.org/), đây là một phần mềm thu thập dữ liệu dựa trên [Open Data Kit](https://opendatakit.org/).
...
[***MẸO:*** Từ điển dữ liệu Kobo có tên biến trong cột "name" của sheet "survey" (khảo sát). Các giá trị có thể có cho mỗi biến được chỉ định trong sheet "choices" (lựa chọn). Trong tab lựa chọn, "name" có giá trị rút gọn và các cột "label::english" và "label::french" có các phiên bản dài thích hợp. Sử dụng hàm **epidict** của package `msf_dict_survey()` để nhập tệp excel từ điển Kobo sẽ định dạng lại tệp này cho bạn để bạn có thể dễ dàng sử dụng để code lại.]{style="color: darkgreen;"}
...
Bộ dữ liệu được nhập bằng hàm `import()` từ package **rio**. Xem chương về [Nhập xuất dữ liệu](https://epirhandbook.com/import-and-export.html) để biết các cách nhập dữ liệu khác nhau.
...
Chúng tôi cũng muốn nhập dữ liệu về quần thể mẫu để có thể tạo ra các trọng số thích hợp. Dữ liệu này có thể ở các định dạng khác nhau, tuy nhiên chúng tôi khuyên bạn nên định dạng như hình bên dưới (dữ liệu này có thể chỉ cần nhập vào excel).
...
Đối với khảo sát cụm, bạn có thể muốn thêm trọng số khảo sát ở cấp cụm. Bạn có thể đọc dữ liệu này như trên. Ngoài ra, nếu chỉ có một vài số đếm, chúng có thể được nhập như dưới đây trong một bảng. Trong mọi trường hợp, bạn sẽ cần có một cột với mã định danh cụm khớp với dữ liệu khảo sát của bạn và một cột khác với số lượng hộ gia đình ở từng cụm.
...
Dưới đây đảm bảo rằng cột ngày ở định dạng thích hợp. Có một số cách khác để thực hiện việc này (xem chương [Làm việc với ngày tháng](https://epirhandbook.com/working-with-dates.html) để biết chi tiết), tuy nhiên sử dụng từ điển để xác định ngày sẽ nhanh chóng và dễ dàng.

Chúng tôi cũng tạo một biến nhóm tuổi bằng cách sử dụng hàm `age_categories()` từ **epikit** - xem phần [Làm sạch dữ liệu](https://epirhandbook.com/cleaner-data-and-core-functions.html#num_cats) của sổ tay để biết chi tiết. Ngoài ra, chúng tôi tạo một biến ký tự xác định các cụm khác nhau nằm trong khu vực nào.

Cuối cùng, chúng tôi mã hóa lại tất cả các biến có/không thành các biến TRUE/FALSE - nếu không, các biến này không thể được sử dụng bởi các hàm tỷ lệ **survey**.
...
Hai cấp độ này được liên kết bởi một mã định danh duy nhất. Đối với bộ dữ liệu Kobo, biến này là "\_index" ở cấp hộ gia đình, khớp với "\_parent_index" ở cấp cá nhân. Thao tác này sẽ tạo các hàng mới cho hộ gia đình với mỗi cá nhân phù hợp, hãy xem phần [Nối dữ liệu](https://epirhandbook.com/joining-data.html) để biết chi tiết.  
...
Đối với các cuộc khảo sát về tỷ lệ tử vong, chúng ta muốn biết mỗi cá nhân đã có mặt tại địa điểm trong bao lâu để có thể tính toán tỷ lệ tử vong thích hợp trong khoảng thời gian nghiên cứu. Điều này không liên quan đến tất cả các cuộc điều tra, nhưng đặc biệt rất quan trọng đối với các cuộc điều tra tử vong vì chúng thường được khảo sát ở các quần thể di động hoặc di cư.  

Để làm điều này, trước tiên chúng ta xác định khoảng thời gian khảo sát, còn được gọi là khoảng thời gian hồi cứu (tức là thời gian mà những người tham gia được yêu cầu báo cáo khi trả lời câu hỏi). Sau đó, chúng ta có thể sử dụng khoảng thời gian này để đặt các ngày không thích hợp thành giá trị bị thiếu, tức là nếu các trường hợp tử vong được báo cáo ngoài khoảng thời gian khảo sát.  
...
Ngày bắt đầu: Sự kiện phù hợp tới sớm nhất trong khoảng thời gian hồi cứu của bạn. Hoặc là ngày bắt đầu thời gian hồi cứu của bạn (mà bạn xác định trước) hoặc ngày sau khi bắt đầu hồi cứu nếu có (ví dụ: đến hoặc sinh)  
...
Điều quan trọng là bạn phải loại bỏ các đối tượng có lỗi trước khi thêm trọng số khảo sát. Ví dụ: nếu bạn có các quan sát với thời gian theo dõi âm, bạn sẽ cần kiểm tra các đối tượng đó (bạn có thể thực hiện việc này với hàm `assert_positive_timespan()` từ **sitrep**. Một điều khác là nếu bạn muốn bỏ các hàng trống (ví dụ: với `drop_na(uid)`) hoặc xóa các bản trùng (xem phần sổ tay về [Loại bỏ trùng lặp] để biết thêm chi tiết). Những đối tượng không có sự đồng ý cũng cần phải bị xóa.  

Trong ví dụ này, chúng tôi lọc các trường hợp muốn loại bỏ và lưu trữ chúng trong một dataframe riêng - theo cách này, chúng tôi có thể mô tả những trường hợp đã bị loại khỏi khảo sát. Sau đó, chúng tôi sử dụng hàm `anti_join()` từ **dplyr** để xóa những trường hợp bị loại bỏ này khỏi dữ liệu khảo sát của chúng tôi.

[***NGUY HIỂM:*** Bạn không được có giá trị trống trong biến trọng số của mình hoặc bất kỳ biến nào có liên quan đến thiết kế khảo sát của bạn (ví dụ: tuổi, giới tính, biến tầng hoặc cụm).]{style="color: red;"}  
...
Tạo đối tượng khảo sát theo thiết kế nghiên cứu của bạn. Sử dụng theo cách tương tự như dataframe để tính toán tỷ lệ trọng số, v.v. Đảm bảo rằng tất cả các biến cần thiết đã được tạo trước bước này.  

Có bốn tùy chọn, hãy tạo thành bình luận trong code những tùy chọn bạn không sử dụng: - Ngẫu nhiên đơn - Phân tầng - Cụm - Cụm phân tầng  

Đối với mẫu này - chúng tôi sẽ giả vờ rằng chúng tôi nhóm cụm các cuộc khảo sát theo hai tầng riêng biệt (khu vực A và B). Vì vậy, để có được ước tính tổng thể, chúng ta cần có trọng số kết hợp của cụm và tầng.  

Như đã đề cập trước đây, có hai packages có sẵn để thực hiện việc này. Package cổ điển là **survey** và một package gọn hơn được gọi là **srvyr** giúp tạo các đối tượng và chức năng theo giao diện tương tự tidyverse. Chúng tôi sẽ trình bày cả hai, nhưng lưu ý rằng hầu hết code trong chương này sẽ sử dụng các đối tượng dựa trên **srvyr**. Một ngoại lệ là package **gtsummary** chỉ chấp nhận các đối tượng **survey**.  

### Package **Survey** 

Package **survey** sử dụng hiệu quả code **base** *R* nên không thể sử dụng các pipes (`%>%`) hoặc cú pháp **dplyr** khác. Với package **survey**, chúng tôi sử dụng hàm `svydesign()` để xác định đối tượng khảo sát với các cụm, trọng số và tầng thích hợp.  
...
Phân tích mô tả và trực quan hóa cơ bản được đề cập nhiều trong các chương khác của cuốn sổ tay, vì vậy chúng tôi sẽ không đi sâu vào nó ở đây. Để biết chi tiết, hãy xem các chương về [Bảng mô tả](https://epirhandbook.com/descriptive-tables.html), [Các kiểm định thống kê cơ bản](https://epirhandbook.com/simple-statistical-tests.html), [Trình bày bảng](https://epirhandbook.com/tables-for-presentation.html), [ggplot cơ bản](https://epirhandbook.com/ggplot-basics.html) and [Báo cáo với R Markdown](https://epirhandbook.com/r-markdown-reports.html).  

Trong phần này, chúng tôi sẽ tập trung vào cách điều tra sai số trong mẫu của bạn và trực quan chúng. Chúng ta cũng sẽ xem xét việc trực quan hóa dòng dân cư trong bối cảnh khảo sát bằng cách sử dụng sơ đồ luồng alluvial/sankey.  
...
Lưu ý rằng các giá trị p này chỉ mang tính chất chỉ dẫn và phần thảo luận mang tính mô tả (hoặc trực quan với tháp tuổi bên dưới) về các phân bố trong mẫu nghiên cứu của bạn so với quần thể gốc quan trọng hơn chính phép kiểm định hai nhóm. Đó là do việc tăng cỡ mẫu sẽ hay dẫn đến những khác biệt có thể không liên quan sau khi áp dụng trọng số vào dữ liệu của bạn.  
...
Tháp dân số (hoặc tuổi-giới) là một cách dễ dàng để trực quan hóa sự phân bố trong quần thể khảo sát của bạn. Cũng nên xem xét việc tạo [bảng mô tả](https://epirhandbook.com/descriptive-tables.html) về độ tuổi và giới tính theo tầng khảo sát. Chúng tôi sẽ biểu diễn bằng package **apyramid** vì nó cho phép các tỷ lệ được áp dụng trọng số sử dụng đối tượng theo thiết kế nghiên cứu của chúng tôi đã tạo ở trên. Các tùy chọn khác để tạo [Tháp dân số](https://epirhandbook.com/demographic-pyramids-and-likert-scales.html) được đề cập nhiều trong chương đó của sổ tay. Chúng tôi cũng sẽ sử dụng một hàm gọn hơn từ **sitrep** được gọi là `plot_age_pyramid()` để lưu một vài dòng code để tạo ra một biểu đồ theo tỷ lệ.  

Giống như với phép kiểm định sự khác biệt giữa hai nhóm chuẩn, được thấy ở trên trong phần sai số chọn mẫu, ở đây chúng ta quan tâm đến việc hình dung liệu quần thể mẫu của chúng ta có khác biệt đáng kể so với quần thể gốc hay không, và liệu trọng số có sửa chữa được sự khác biệt này hay không. Để làm điều này, chúng tôi sẽ sử dụng package **patchwork** để hiển thị các hình ảnh trực quan **ggplot** song song với nhau; chi tiết xem phần kết hợp các đồ thị trong chương [Các tips với ggplot](https://epirhandbook.com/ggplot-tips.html?q=patch#combine-plots) của sổ tay. Chúng tôi sẽ trực quan hóa quần thể gốc, quần thể khảo sát không trọng số và quần thể khảo sát có trọng số. Bạn cũng có thể xem xét trực quan theo từng tầng của khảo sát - bằng cách sử dụng đối số `stack_by  = "health_district"` trong ví dụ của chúng tôi ở đây (xem `?plot_age_pyramid` để biết chi tiết).  
...
Phần này sẽ trình bày chi tiết cách tạo bảng cho số lượng và tỷ lệ được áp dụng trọng số, với khoảng tin cậy và hệ số thiết kế liên quan. Có bốn tùy chọn khác nhau sử dụng các hàm từ các packages sau: **survey**, **srvyr**, **sitrep** và **gtsummary**. Để tạo ra một bảng dịch tễ học tiêu chuẩn với code tối giản, chúng tôi khuyên bạn nên dùng hàm **sitrep** - là một hàm có code gọn hơn của **srvyr**; Tuy nhiên, lưu ý rằng hàm này chưa có trên CRAN và có thể thay đổi trong tương lai. Mặt khác, code của **survey** có thể là code ổn định nhất trong thời gian dài, trong khi **srvyr** sẽ phù hợp nhất trong các luồng việc tidyverse. Mặc dù các hàm **gtsummary** có rất nhiều tiềm năng, nhưng chúng vẫn đang được thử nghiệm và chưa hoàn thiện tại thời điểm viết chương này.  

### Package **survey**

Chúng ta có thể sử dụng hàm `svyciprop()` từ **survey** để lấy tỷ lệ có trọng số và khoảng tin cậy 95% đi kèm. Một hệ số thiết kế thích hợp có thể được trích xuất bằng cách sử dụng hàm `svymean()` chứ không phải hàm `svyprop()`. Cần lưu ý rằng `svyprop()` dường như chỉ chấp nhận các biến từ 0 đến 1 (hoặc TRUE/FALSE), vì vậy các biến phân loại sẽ không hoạt động.  

[***LƯU Ý:*** Các hàm từ **survey** cũng chấp nhận các đối tượng thiết kế **srvyr**, nhưng ở đây chúng tôi đã sử dụng đối tượng thiết kế **survey** chỉ cho tính nhất quán]{style="color: black;"}  
...
Chúng ta có thể kết hợp các hàm từ **survey** được hiển thị ở trên với một hàm mà chúng ta tự định nghĩa bên dưới, được gọi là `svy_prop`; và sau đó chúng ta có thể sử dụng hàm đó cùng với `map()` từ package purrr để lặp qua một số biến và tạo một bảng. Xem chương [Lặp](https://epirhandbook.com/iteration-loops-and-lists.html) để biết chi tiết về **purrr**.  
...
Với **srvyr** chúng ta có thể sử dụng cú pháp **dplyr** để tạo một bảng. Lưu ý rằng hàm `survey_mean()` được sử dụng, đối số tỷ lệ được chỉ định và hàm tương tự cũng được sử dụng để tính toán hệ số thiết kế. Điều này là do **srvyr** bao gồm cả hai hàm `svyciprop()` và `svymean()` của package **survey**, được sử dụng trong phần trên.  
...
Ở đây, chúng ta cũng có thể viết một hàm để sau đó lặp qua nhiều biến bằng cách sử dụng package **purrr**. Xem chương [Lặp](https://epirhandbook.com/iteration-loops-and-lists.html) của sổ tay để biết chi tiết về **purrr**.  
